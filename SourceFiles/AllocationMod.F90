module AllocationMod
  ...
  !#fates_py !#py use ELMFatesInterfaceMod  , only : hlm_fates_interface_type
...
contains
  ...
  !-----------------------------------------------------------------------
!#py   subroutine readCNAllocParams ( ncid )
!#py     !
!#py     ! !USES:
...
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py     character(len=32)  :: subname = 'AllocParamsType'
...
!#py     !-----------------------------------------------------------------------
!#py     allocate(AllocParamsInst%bdnr              )
!#py     allocate(AllocParamsInst%dayscrecover      )
!#py     allocate(AllocParamsInst%compet_plant_no3  )
!#py     allocate(AllocParamsInst%compet_plant_nh4  )
!#py     allocate(AllocParamsInst%compet_decomp_no3 )
!#py     allocate(AllocParamsInst%compet_decomp_nh4 )
!#py     allocate(AllocParamsInst%compet_denit      )
!#py     allocate(AllocParamsInst%compet_nit        )
...
!#py     AllocParamsInst%bdnr=tempr
!#py 
!#py     tString='dayscrecover'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%dayscrecover=tempr
!#py 
!#py     tString='compet_plant_no3'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%compet_plant_no3=tempr
!#py 
!#py     tString='compet_plant_nh4'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%compet_plant_nh4=tempr
!#py 
!#py     tString='compet_decomp_no3'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%compet_decomp_no3=tempr
!#py 
!#py     tString='compet_decomp_nh4'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%compet_decomp_nh4=tempr
!#py 
!#py     tString='compet_denit'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%compet_denit=tempr
!#py 
!#py     tString='compet_nit'
!#py     call ncd_io(varname=trim(tString),data=tempr, flag='read', ncid=ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     AllocParamsInst%compet_nit=tempr
!#py 
!#py   end subroutine readCNAllocParams
  ...
  subroutine AllocationInit ( bounds)
    ...
    case default
       !#py write(iulog,*) 'Supplemental Nitrogen flag (suplnitro) can only be: ', &
            !#py suplnNon, ' or ', suplnAll
       !#py call endrun(msg='ERROR: supplemental Nitrogen flag is not correct'//&
...
!       end if
 ...
 subroutine calc_nuptake_prof(num_soilc, filter_soilc, cnstate_vars, nuptake_prof)
    ...
    real(r8):: sminn_vr_loc(num_soilc, 1:nlevdecomp)
    ...
    associate( &
         ...
         do j = 1, nlevdecomp
            do fc=1,num_soilc
               ...
               sminn_vr_loc(fc,j) = smin_no3_vr(c,j) + smin_nh4_vr(c,j)

               if(use_pflotran .and. pf_cmode) then
                    sminn_tot(fc) = sminn_tot(fc) + sminn_vr_loc(fc,j) * dzsoi_decomp(j) &
         ...
         do j = 1, nlevdecomp
            do fc=1,num_soilc
               ...
               if (sminn_tot(fc)  >  0._r8) then
                  if(use_pflotran .and. pf_cmode) then
                     nuptake_prof(fc,j) = sminn_vr_loc(fc,j) / sminn_tot(fc) &
                        *(nfixation_prof(c,j)*dzsoi_decomp(j))         ! weighted by froot fractions in annual max. active layers
                  else
                     nuptake_prof(fc,j) = sminn_vr_loc(fc,j) / sminn_tot(fc)   !original:  nuptake_prof(c,j) = sminn_vr(c,j) / sminn_tot(c)
                  end if
               else
...
!-------------------------------------------------------------------------------------------------
  subroutine calc_puptake_prof(num_soilc, filter_soilc, cnstate_vars, puptake_prof)
    ...
    integer :: c,j,fc                                            !indices
    real(r8):: solutionp_tot(num_soilc)
    ...
    associate( &
         ...
         do j = 1, nlevdecomp
            do fc=1,num_soilc
               c = filter_soilc(fc)
               solutionp_tot(fc) = solutionp_tot(fc) + solutionp_vr(c,j) * dzsoi_decomp(j)
            end do
         end do
