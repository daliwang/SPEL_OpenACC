module UrbanParamsType
    ...
    do l = bounds%begl,bounds%endl
       ...
       if (lun_pp%urbpoi(l)) then
          ...
          if (abs(sumvf-1._r8) > 1.e-06_r8 ) then
             !#py write (iulog,*) 'urban road view factor error',sumvf
          ...
          if (abs(sumvf-1._r8) > 1.e-06_r8 ) then
             !#py write (iulog,*) 'urban wall view factor error',sumvf
             !#py write (iulog,*) 'elm model is stopping'
             !#py !#py call endrun(decomp_index=l, elmlevel=namel, msg=errmsg(__FILE__, __LINE__))
          endif
          ...
          if (use_vancouver) then
             ...
             !#py lun_pp%z_0_town(l) = lun_pp%ht_roof(l) * (1._r8 - lun_pp%z_d_town(l) / lun_pp%ht_roof(l)) * &
                  !#py exp(-1.0_r8 * (0.5_r8 * beta * C_d / vkc**2 * &
                  !#py (1 - lun_pp%z_d_town(l) / lun_pp%ht_roof(l)) * frontal_ai)**(-0.5_r8))
          end if
  ...
  subroutine UrbanInput(begg, endg, mode)
    ...
    if (mode == 'initialize') then
       ...
       !#py !#py call ncd_inqdid(ncid, 'numurbl', dimid, dimexist=has_numurbl)

       ! If file doesn't have numurbl, then it is old-format urban;
       ! in this case, set nlevurb to zero
       if (.not. has_numurbl) then
         nlevurb = 0
         !#py write(iulog,*)'PCT_URBAN is not multi-density, nlevurb set to 0'
       end if
       ...
       allocate(urbinp%canyon_hwr(begg:endg,max_topounits, numurbl), &  
                ...
                stat=ier)
       if (ier /= 0) then
          !#py !#py call endrun(msg="Allocation error "//errmsg(__FILE__, __LINE__))
       endif
       ...
       if (.not. has_topounit) then
          if (ldomain%ns /= ns .or. ldomain%ni /= ni .or. ldomain%nj /= nj) then
             !#py write(iulog,*)trim(subname), 'ldomain and input file do not match dims '
             !#py write(iulog,*)trim(subname), 'ldomain%ni,ni,= ',ldomain%ni,ni
             !#py write(iulog,*)trim(subname), 'ldomain%nj,nj,= ',ldomain%nj,nj
             !#py write(iulog,*)trim(subname), 'ldomain%ns,ns,= ',ldomain%ns,ns
       ...
       if (nlevurb_i /= nlevurb) then
          !#py write(iulog,*)trim(subname)// ': parameter nlevurb= ',nlevurb, &
               !#py !#py 'does not equal input dataset nlevurb= ',nlevurb_i
          !#py !#py call endrun(msg=errmsg(__FILE__, __LINE__))
       endif
       ...
       if (numrad_i /= numrad) then
          !#py write(iulog,*)trim(subname)// ': parameter numrad= ',numrad, &
               !#py !#py 'does not equal input dataset numrad= ',numrad_i
          !#py !#py call endrun(msg=errmsg(__FILE__, __LINE__))
       endif
       !#py call ncd_inqdid(ncid, 'numurbl', dimid)
       !#py call ncd_inqdlen(ncid, dimid, numurbl_i)
       if (numurbl_i /= numurbl) then
          !#py write(iulog,*)trim(subname)// ': parameter numurbl= ',numurbl, &
               !#py !#py 'does not equal input dataset numurbl= ',numurbl_i
          !#py !#py call endrun(msg=errmsg(__FILE__, __LINE__))
       endif
       !#py call ncd_io(ncid=ncid, varname='CANYON_HWR', flag='read', data=urbinp%canyon_hwr,&
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg='ERROR: CANYON_HWR NOT on fsurdat file '//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='WTLUNIT_ROOF', flag='read', data=urbinp%wtlunit_roof, &
            !#py !#py dim1name=grlnd,  readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: WTLUNIT_ROOF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='WTROAD_PERV', flag='read', data=urbinp%wtroad_perv, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: WTROAD_PERV NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='EM_ROOF', flag='read', data=urbinp%em_roof, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: EM_ROOF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='EM_IMPROAD', flag='read', data=urbinp%em_improad, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: EM_IMPROAD NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='EM_PERROAD', flag='read', data=urbinp%em_perroad, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: EM_PERROAD NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='EM_WALL', flag='read', data=urbinp%em_wall, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: EM_WALL NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='HT_ROOF', flag='read', data=urbinp%ht_roof, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: HT_ROOF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='WIND_HGT_CANYON', flag='read', data=urbinp%wind_hgt_canyon, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: WIND_HGT_CANYON NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='THICK_WALL', flag='read', data=urbinp%thick_wall, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: THICK_WALL NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='THICK_ROOF', flag='read', data=urbinp%thick_roof, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: THICK_ROOF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='NLEV_IMPROAD', flag='read', data=urbinp%nlev_improad, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: NLEV_IMPROAD NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='T_BUILDING_MIN', flag='read', data=urbinp%t_building_min, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: T_BUILDING_MIN NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='T_BUILDING_MAX', flag='read', data=urbinp%t_building_max, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: T_BUILDING_MAX NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_IMPROAD_DIR', flag='read', data=urbinp%alb_improad_dir, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not.readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_IMPROAD_DIR NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_IMPROAD_DIF', flag='read', data=urbinp%alb_improad_dif, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not.readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_IMPROAD_DIF NOT on fsurdat file'//errmsg(__FILE__, __LINE__) )
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_PERROAD_DIR', flag='read',data=urbinp%alb_perroad_dir, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_PERROAD_DIR NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_PERROAD_DIF', flag='read',data=urbinp%alb_perroad_dif, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_PERROAD_DIF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_ROOF_DIR', flag='read', data=urbinp%alb_roof_dir,  &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_ROOF_DIR NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_ROOF_DIF', flag='read', data=urbinp%alb_roof_dif,  &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_ROOF_DIF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_WALL_DIR', flag='read', data=urbinp%alb_wall_dir, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_WALL_DIR NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='ALB_WALL_DIF', flag='read', data=urbinp%alb_wall_dif, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: ALB_WALL_DIF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='TK_IMPROAD', flag='read', data=urbinp%tk_improad, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: TK_IMPROAD NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='TK_ROOF', flag='read', data=urbinp%tk_roof, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: TK_ROOF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='TK_WALL', flag='read', data=urbinp%tk_wall, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: TK_WALL NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='CV_IMPROAD', flag='read', data=urbinp%cv_improad, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: CV_IMPROAD NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='CV_ROOF', flag='read', data=urbinp%cv_roof, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: CV_ROOF NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if

       !#py call ncd_io(ncid=ncid, varname='CV_WALL', flag='read', data=urbinp%cv_wall, &
            !#py !#py dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          !#py !#py call endrun( msg=' ERROR: CV_WALL NOT on fsurdat file'//errmsg(__FILE__, __LINE__))
       end if
       ...
       deallocate(urbinp%canyon_hwr, &
                  ...
                  stat=ier)
       if (ier /= 0) then
          !#py !#py call endrun(msg='initUrbanInput: deallocation error '//errmsg(__FILE__, __LINE__))
       end if
    else
       !#py write(iulog,*)'initUrbanInput error: mode ',trim(mode),' not supported '
       !#py !#py call endrun(msg=errmsg(__FILE__, __LINE__))
    end if
  ...
  subroutine CheckUrban(begg, endg, pcturb, caller,ntpu)
       ...
       do n = 1, numurbl
          if ( pcturb(nl,t,n) > 0.0_r8 ) then
             if ( .not. urban_valid(nl,t) .or. &
                  !#py urbinp%canyon_hwr(nl,t,n)            <= 0._r8 .or. &
                  !#py urbinp%em_improad(nl,t,n)            <= 0._r8 .or. &
                  !#py urbinp%em_perroad(nl,t,n)            <= 0._r8 .or. &
                  !#py urbinp%em_roof(nl,t,n)               <= 0._r8 .or. &
                  !#py urbinp%em_wall(nl,t,n)               <= 0._r8 .or. &
                  !#py urbinp%ht_roof(nl,t,n)               <= 0._r8 .or. &
                  !#py urbinp%thick_roof(nl,t,n)            <= 0._r8 .or. &
                  !#py urbinp%thick_wall(nl,t,n)            <= 0._r8 .or. &
                  !#py urbinp%t_building_max(nl,t,n)        <= 0._r8 .or. &
                  !#py urbinp%t_building_min(nl,t,n)        <= 0._r8 .or. &
                  !#py urbinp%wind_hgt_canyon(nl,t,n)       <= 0._r8 .or. &
                  !#py urbinp%wtlunit_roof(nl,t,n)          <= 0._r8 .or. &
                  !#py urbinp%wtroad_perv(nl,t,n)           <= 0._r8 .or. &
                  !#py any(urbinp%alb_improad_dir(nl,t,n,:) <= 0._r8) .or. &
                  !#py any(urbinp%alb_improad_dif(nl,t,n,:) <= 0._r8) .or. &
                  !#py any(urbinp%alb_perroad_dir(nl,t,n,:) <= 0._r8) .or. &
                  !#py any(urbinp%alb_perroad_dif(nl,t,n,:) <= 0._r8) .or. &
                  !#py any(urbinp%alb_roof_dir(nl,t,n,:)    <= 0._r8) .or. &
                  !#py any(urbinp%alb_roof_dif(nl,t,n,:)    <= 0._r8) .or. &
                  !#py any(urbinp%alb_wall_dir(nl,t,n,:)    <= 0._r8) .or. &
                  !#py any(urbinp%alb_wall_dif(nl,t,n,:)    <= 0._r8) .or. &
                  !#py any(urbinp%tk_roof(nl,t,n,:)         <= 0._r8) .or. &
                  !#py any(urbinp%tk_wall(nl,t,n,:)         <= 0._r8) .or. &
                  !#py any(urbinp%cv_roof(nl,t,n,:)         <= 0._r8) .or. &
                  !#py any(urbinp%cv_wall(nl,t,n,:)         <= 0._r8)) then
                found = .true.
                nindx = nl
                dindx = n
                exit
                ...
                if (urbinp%nlev_improad(nl,t,n) > 0) then
                   ...
                   !#py if ( any(urbinp%tk_improad(nl,t,n,1:nlev) <= 0._r8) .or. &
                        !#py any(urbinp%cv_improad(nl,t,n,1:nlev) <= 0._r8)) then
                      found = .true.
                      nindx = nl
                      dindx = n
                      exit
    ...
    if ( found ) then
       !#py write(iulog,*) trim(caller), ' ERROR: no valid urban data for nl=',nindx
       !#py write(iulog,*)'density type:    ',dindx
       !#py write(iulog,*)'urban_valid:     ',urban_valid(nindx,:)
       !#py write(iulog,*)'canyon_hwr:      ',urbinp%canyon_hwr(nindx,:,dindx)
       !#py write(iulog,*)'em_improad:      ',urbinp%em_improad(nindx,:,dindx)
       !#py write(iulog,*)'em_perroad:      ',urbinp%em_perroad(nindx,:,dindx)
       !#py write(iulog,*)'em_roof:         ',urbinp%em_roof(nindx,:,dindx)
       !#py write(iulog,*)'em_wall:         ',urbinp%em_wall(nindx,:,dindx)
       !#py write(iulog,*)'ht_roof:         ',urbinp%ht_roof(nindx,:,dindx)
       !#py write(iulog,*)'thick_roof:      ',urbinp%thick_roof(nindx,:,dindx)
       !#py write(iulog,*)'thick_wall:      ',urbinp%thick_wall(nindx,:,dindx)
       !#py write(iulog,*)'t_building_max:  ',urbinp%t_building_max(nindx,:,dindx)
       !#py write(iulog,*)'t_building_min:  ',urbinp%t_building_min(nindx,:,dindx)
       !#py write(iulog,*)'wind_hgt_canyon: ',urbinp%wind_hgt_canyon(nindx,:,dindx)
       !#py write(iulog,*)'wtlunit_roof:    ',urbinp%wtlunit_roof(nindx,:,dindx)
       !#py write(iulog,*)'wtroad_perv:     ',urbinp%wtroad_perv(nindx,:,dindx)
       !#py write(iulog,*)'alb_improad_dir: ',urbinp%alb_improad_dir(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_improad_dif: ',urbinp%alb_improad_dif(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_perroad_dir: ',urbinp%alb_perroad_dir(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_perroad_dif: ',urbinp%alb_perroad_dif(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_roof_dir:    ',urbinp%alb_roof_dir(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_roof_dif:    ',urbinp%alb_roof_dif(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_wall_dir:    ',urbinp%alb_wall_dir(nindx,:,dindx,:)
       !#py write(iulog,*)'alb_wall_dif:    ',urbinp%alb_wall_dif(nindx,:,dindx,:)
       !#py write(iulog,*)'tk_roof:         ',urbinp%tk_roof(nindx,:,dindx,:)
       !#py write(iulog,*)'tk_wall:         ',urbinp%tk_wall(nindx,:,dindx,:)
       !#py write(iulog,*)'cv_roof:         ',urbinp%cv_roof(nindx,:,dindx,:)
       !#py write(iulog,*)'cv_wall:         ',urbinp%cv_wall(nindx,:,dindx,:)
       if (urbinp%nlev_improad(nindx,1,dindx) > 0) then
          nlev = urbinp%nlev_improad(nindx,1,dindx)
          !#py write(iulog,*)'tk_improad: ',urbinp%tk_improad(nindx,:,dindx,1:nlev)
          !#py write(iulog,*)'cv_improad: ',urbinp%cv_improad(nindx,:,dindx,1:nlev)
       end if
       !#py !#py call endrun(msg=errmsg(__FILE__, __LINE__))
    end if
