module lnd2glcMod
  ...
  type, public :: lnd2glc_type
   ...
   contains
     ...
     !#py procedure, private :: InitAllocate
...
contains
  ...
  subroutine Init(this, bounds)
    ...
    !#py call this%InitAllocate(bounds)
    !#py call this%InitHistory(bounds)

  end subroutine Init

  !------------------------------------------------------------------------
!#py   subroutine InitAllocate(this, bounds)
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Initialize land variables required by glc
!#py     !
!#py     ! !USES:
...
!#py 
!#py     allocate(this%tsrf_grc(begg:endg,0:maxpatch_glcmec)) ; this%tsrf_grc(:,:)=0.0_r8
!#py     allocate(this%topo_grc(begg:endg,0:maxpatch_glcmec)) ; this%topo_grc(:,:)=0.0_r8
!#py     allocate(this%qice_grc(begg:endg,0:maxpatch_glcmec)) ; this%qice_grc(:,:)=0.0_r8
!#py 
!#py   end subroutine InitAllocate
...
!#py        this%qice_grc(begg:endg,0:maxpatch_glcmec) = spval
!#py        ! For this and the following fields, set up a pointer to the field simply for the
!#py        ! sake of changing the indexing, so that levels start with an index of 1, as is
!#py        ! assumed by histFileMod - so levels go 1:(nec+1) rather than 0:nec
!#py        data2dptr => this%qice_grc(:,0:maxpatch_glcmec)
!#py        call hist_addfld2d (fname='QICE_FORC', units='mm/s', type2d='elevclas', &
!#py             avgflag='A', long_name='qice forcing sent to GLC', &
!#py             ptr_lnd=data2dptr, default='inactive')
!#py 
!#py        this%tsrf_grc(begg:endg,0:maxpatch_glcmec) = spval
!#py        data2dptr => this%tsrf_grc(:,0:maxpatch_glcmec)
!#py        call hist_addfld2d (fname='TSRF_FORC', units='K', type2d='elevclas', &
!#py             avgflag='A', long_name='surface temperature sent to GLC', &
!#py             ptr_lnd=data2dptr, default='inactive')
!#py 
!#py        this%topo_grc(begg:endg,0:maxpatch_glcmec) = spval
!#py        data2dptr => this%topo_grc(:,0:maxpatch_glcmec)
!#py        call hist_addfld2d (fname='TOPO_FORC', units='m', type2d='elevclas', &
!#py             avgflag='A', long_name='topograephic height sent to GLC', &
!#py             ptr_lnd=data2dptr, default='inactive')
!#py     end if
!#py 
  ...
  subroutine update_lnd2glc(this, bounds, num_do_smb_c, filter_do_smb_c,  init)
    ...
    do fc = 1, num_do_smb_c
      ...
      if (fields_assigned(g,n)) then
         !#py write(iulog,*) subname//' ERROR: attempt to assign coupling fields twice for the same index.'
         !#py write(iulog,*) 'One possible cause is having multiple columns in the istsoil landunit,'
         !#py write(iulog,*) 'which this routine cannot handle.'
         !#py write(iulog,*) 'g, n = ', g, n
      ...
      if (.not. init) then
         ...
         if ( abs(this%qice_grc(g,n)) > 1.0_r8) then
            !#py write(iulog,*) 'WARNING: qice out of bounds: g, n, qice =', g, n, this%qice_grc(g,n)
         end if
      end if
