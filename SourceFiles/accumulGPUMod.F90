module accumulGPUMod
  ...
  contains
    ...
    subroutine update_accum_field_sl (name, field, nstep)
      ...
      ! !ARGUMENTS:
      implicit none
      character(len=50), intent(in) :: name     !field name
      ...
      nf = 0
      i = 1
      ITERATION: do while (i< naccflds+1)
        print *, i, name, accum_field_gpu(i)%name
         if (name == accum_field_gpu(i)%name) then
           nf = i
           i = naccflds+2
         end if
         i = i +1
      ...
      if (nf == 0) then
         print *, 'UPDATE_ACCUM_FIELD_SL error: field name ',name,' not found'
      endif
    ...
    subroutine update_accum_field_ml (name, field, nstep)
      ...
      nf = 0
      i = 1
      do while(nf == 0 .and. i <= naccflds)
         if (name == accum_field_gpu(i)%name) nf = i
         i = i+1
      end do
      if (nf == 0) then
         print *, 'UPDATE_ACCUM_FIELD_ML error: field name ',name,' not found'
      endif
    ...
    subroutine extract_accum_field_sl (name, field, nstep)
      ...
      if (nf == 0) then
         print *, 'EXTRACT_ACCUM_FIELD_SL error: field name not found'
      endif
    ...
    subroutine extract_accum_field_ml (name, field, nstep)
     ...
     subroutine atm2lnd_UpdateAccVars (atm2lnd_vars, bounds, nstep)
      ...
      ! Accumulate and extract forc_solad24 & forc_solad240
       ...
       call extract_accum_field_gpu (name, atm2lnd_vars%fsd240_patch, nstep)
       name = 'FSD24'
       call update_accum_field_gpu  ( name , rbufslp, nstep)
       call extract_accum_field_gpu ( name , atm2lnd_vars%fsd24_patch, nstep)
       ...
       name = 'FSI24'
       call update_accum_field_gpu  ( name, rbufslp , nstep)
       call extract_accum_field_gpu ( name , atm2lnd_vars%fsi24_patch      , nstep)
       name = 'FSI240'
       call update_accum_field_gpu  ( name, rbufslp , nstep)
       call extract_accum_field_gpu ( name, atm2lnd_vars%fsi240_patch     , nstep)
       do p = begp,endp
          c = veg_pp%column(p)
       ...
       if (use_cn) then
          ...
          call update_accum_field_gpu  (name, rbufslp, nstep)
          call extract_accum_field_gpu (name, atm2lnd_vars%prec60_patch, nstep)

          ! Accumulate and extract PREC10 (accumulates total precipitation as 10-day running mean)
          name = 'PREC10'
          call update_accum_field_gpu  ( name, rbufslp, nstep)
          call extract_accum_field_gpu ( name, atm2lnd_vars%prec10_patch, nstep)
       end if
       if (use_fates) then
         name = 'PREC24'
          call update_accum_field_gpu  (name, rbufslp, nstep)
     ...
     subroutine update_acc_vars_top_afGPU (top_af, bounds, nstep)
       ...
       if (use_cn) then
          ...
          call extract_accum_field_gpu (NAME, top_af%prec60d, nstep)
          ! Accumulate and extract PREC10D (accumulates total precipitation as 10-day running mean)
          name = 'PREC10D'
          call update_accum_field_gpu  (name, rbufslt, nstep)
          call extract_accum_field_gpu (name, top_af%prec10d, nstep)
       end if
       if (use_fates) then
         name = 'PREC24H'
          call update_accum_field_gpu  (name, rbufslt, nstep)
  ...
  subroutine update_acc_vars_veg_es_GPU(veg_es, bounds, dtime, nstep, &
    ...
    name = 'T10'
    call update_accum_field_gpu  (name, veg_es%t_ref2m, nstep)
    call extract_accum_field_gpu (name, veg_es%t_a10  , nstep)
    name = 'T_VEG24'
    call update_accum_field_gpu  (NAME , rbufslp       , nstep)
    call extract_accum_field_gpu (NAME , veg_es%t_veg24  , nstep)
    name = 'T_VEG240'
    call update_accum_field_gpu  (name, rbufslp       , nstep)
    call extract_accum_field_gpu (name, veg_es%t_veg240 , nstep)
    ...
    name = 'TREFAV'
    call update_accum_field_gpu  (name , veg_es%t_ref2m, nstep)
    call extract_accum_field_gpu (name , rbufslp, nstep)
    do p = begp,endp
    ...
    ! an initial run at the first time step the accumulator is called
    name = 'TREFAV_U'
    call update_accum_field_gpu  (NAME , veg_es%t_ref2m_u, nstep)
    call extract_accum_field_gpu (NAME , rbufslp, nstep)
    do p = begp,endp
       l = veg_pp%landunit(p)
       if (rbufslp(p) /= spval) then
    ...
    ! an initial run at the first time step the accumulator is called
    NAME = 'TREFAV_R'
    call update_accum_field_gpu  (name, veg_es%t_ref2m_r, nstep)
    call extract_accum_field_gpu (name, rbufslp, nstep)
    do p = begp,endp
       l = veg_pp%landunit(p)
       if (rbufslp(p) /= spval) then
    ...
    if ( crop_prog )then
       ...
       name = 'TDM10'
       call update_accum_field_gpu  (name, rbufslp, nstep)
       call extract_accum_field_gpu (name, veg_es%t_a10min, nstep)
       ...
       name = 'TDM5'
       call update_accum_field_gpu  (name, rbufslp, nstep)
       call extract_accum_field_gpu (name, veg_es%t_a5min, nstep)
       ...
       name = 'GDD0'
       call update_accum_field_gpu  (name, rbufslp, nstep)
       call extract_accum_field_gpu (name, veg_es%gdd0, nstep)
       ...
       name = 'GDD8'
       call update_accum_field_gpu  (name, rbufslp, nstep)
       call extract_accum_field_gpu (name, veg_es%gdd8, nstep)
       ...
       name = 'GDD10'
       call update_accum_field_gpu  (name , rbufslp, nstep)
       call extract_accum_field_gpu (name , veg_es%gdd10, nstep)
    end if
  ...
  !-----------------------------------------------------------------------
    subroutine CanopyState_UpdateAccVars (canopystate_vars, bounds, dtime,nstep)
      ...
      name = 'FSUN24'
      call update_accum_field_gpu  (NAME , rbufslp              , nstep)
      call extract_accum_field_gpu (NAME , canopystate_vars%fsun24_patch    , nstep)
      name = 'FSUN240'
      call update_accum_field_gpu  (name, rbufslp              , nstep)
      call extract_accum_field_gpu (name, canopystate_vars%fsun240_patch   , nstep)
      ...
      name = 'LAIP'
      call update_accum_field_gpu  (name, rbufslp                 , nstep)
      call extract_accum_field_gpu (name, canopystate_vars%elai_p_patch       , nstep)
    ...
    subroutine crop_vars_UpdateAccVars(crop_vars, bounds, dtime, nstep)
      ...
      name = 'GDDPLANT'
      call update_accum_field_gpu  (name, rbufslp, nstep)
      call extract_accum_field_gpu (name, crop_vars%gddplant_patch, nstep)
      ...
      name = 'GDDTSOI'
      call update_accum_field_gpu  (name, rbufslp, nstep)
      call extract_accum_field_gpu (name, crop_vars%gddtsoi_patch, nstep)
    ...
    !-----------------------------------------------------------------------
      subroutine CNState_UpdateAccVars (cnstate_vars, bounds, nstep)
        ...
        name = 'nlim_m'
        call update_accum_field_gpu  (name , rbufslp             , nstep)
        call extract_accum_field_gpu (name , cnstate_vars%cn_scalar_runmean  , nstep)
        do p = begp,endp
           rbufslp(p) = cnstate_vars%cp_scalar(p)
        end do
        name = 'plim_m'
        call update_accum_field_gpu  (name , rbufslp             , nstep)
        call extract_accum_field_gpu (name , cnstate_vars%cp_scalar_runmean  , nstep)
