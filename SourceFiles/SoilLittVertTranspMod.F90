module SoilLittVertTranspMod
  ...
  !#py public :: readSoilLittVertTranspParams
...
contains
  ...
  !-----------------------------------------------------------------------
!#py   subroutine readSoilLittVertTranspParams ( ncid )
!#py     !
!#py     use ncdio_pio   , only : file_desc_t,ncd_io
!#py     !
!#py     type(file_desc_t),intent(inout) :: ncid   ! pio netCDF file id
!#py     !
!#py     character(len=32)  :: subname = 'SoilLittVertTranspType'
...
!#py     !
!#py     allocate(SoilLittVertTranspParamsInst%som_diffus                )
!#py     allocate(SoilLittVertTranspParamsInst%cryoturb_diffusion_k      )
!#py     allocate(SoilLittVertTranspParamsInst%max_altdepth_cryoturbation)
!#py     tString='som_diffus'
...
!#py     !SoilLittVertTranspParamsInst%som_diffus=tempr
!#py     ! FIX(SPM,032414) - can't be pulled out since division makes things not bfb
!#py     SoilLittVertTranspParamsInst%som_diffus = 1e-4_r8 / (secspday * 365._r8)
!#py 
!#py     tString='cryoturb_diffusion_k'
!#py     call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     !SoilLittVertTranspParamsInst%cryoturb_diffusion_k=tempr
!#py     !FIX(SPM,032414) Todo.  This constant cannot be on file since the divide makes things
!#py     !SPM Todo.  This constant cannot be on file since the divide makes things
!#py     !not bfb
!#py 
!#py     SoilLittVertTranspParamsInst%cryoturb_diffusion_k = 5e-4_r8 / (secspday * 365._r8)  ! [m^2/sec] = 5 cm^2 / yr = 1m^2 / 200 yr
!#py 
!#py     tString='max_altdepth_cryoturbation'
!#py     call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
!#py     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
!#py     SoilLittVertTranspParamsInst%max_altdepth_cryoturbation=tempr
!#py 
!#py     !$acc enter data copyin(SoilLittVertTranspParamsInst)
!#py   end subroutine readSoilLittVertTranspParams
