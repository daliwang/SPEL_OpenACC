...
!#py #include "shr_assert.h"
  ...
  !#py use clm_time_manager , only : get_days_per_year, get_step_size_real, is_beg_curr_year
  !#py use clm_time_manager , only : get_curr_yearfrac, get_prev_yearfrac, get_prev_date
  !#py use clm_time_manager , only : is_first_step
  !
  implicit none
  private
  ...
  !#py character(len=*), parameter, private :: sourcefile = &
       !#py __FILE__
...
contains
  ...
  subroutine set_curr_delta(this, bounds, delta)
   ...
   !             !#py write(iulog,*) subname//' ERROR: found unexpected non-zero delta mid-year'
   !             !#py write(iulog,*) 'Dribbler name: ', trim(this%name)
   !             !#py write(iulog,*) 'i, delta = ', i, this%amount_from_this_timestep(i)
   !             !#py call get_prev_date(yr, mon, day, tod)
   !             !#py write(iulog,*) 'Start of time step date (yr, mon, day, tod) = ', &
   !                  !#py yr, mon, day, tod
   !             !#py write(iulog,*) 'This indicates that some non-zero flux was generated at a time step'
   !             !#py write(iulog,*) 'other than the first time step of the year, which this dribbler was told not to expect.'
   !             !#py write(iulog,*) 'If this non-zero mid-year delta is expected, then you can suppress this error'
   !             !#py write(iulog,*) 'by setting allows_non_annual_delta to .true. when constructing this dribbler.'
   !             !#py call endrun(decomp_index=i, elmlevel=this%name_subgrid, &
   !                  !#py msg=subname//': found unexpected non-zero delta mid-year: ' // &
   !                  !#py !#py errMsg(sourcefile, __LINE__))
   !          end if
   !       end do
   !    end if
   ! end if
  ...
  subroutine get_curr_flux(this, bounds, flux)
    ...
    !#py secs_per_year = get_days_per_year() * secspday
    !#py dtime = get_step_size_real()
...
!#py     !
!#py     ! !ARGUMENTS:
!#py     class(annual_flux_dribbler_type), intent(inout) :: this
...
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py     character(len=:), allocatable :: restname  ! name of field on restart file
!#py     logical :: readvar
!#py 
!#py     character(len=*), parameter :: subname = 'Restart'
!#py     !-----------------------------------------------------------------------
!#py 
!#py     restname = trim(this%name) // '_amt_to_dribble'
!#py     call restartvar(ncid=ncid, flag=flag, varname=restname, xtype=ncd_double, &
!#py          dim1name = this%dim1name, &
!#py          long_name = 'total amount to dribble over the year for ' // trim(this%name), &
!#py          units = trim(this%units), &
!#py          interpinic_flag = 'interp', &
!#py          readvar = readvar, &
!#py          data = this%amount_to_dribble)
!#py 
!#py   end subroutine Restart
  ...
  subroutine set_metadata(this, name, units, allows_non_annual_delta)
    ...
    if (len_trim(name) > name_maxlen) then
       !#py write(iulog,*) subname // ': name too long'
       !#py write(iulog,*) trim(name) // ' exceeds max length: ', name_maxlen
       !#py call endrun(msg=subname // ': name too long: ' // &
            !#py !#py errMsg(sourcefile, __LINE__))
    end if
    this%name = trim(name)

    if (len_trim(units) > units_maxlen) then
       !#py write(iulog,*) subname // ': units too long'
       !#py write(iulog,*) trim(units) // ' exceeds max length: ', units_maxlen
       !#py call endrun(msg=subname // ': units too long: ' // &
            !#py !#py errMsg(sourcefile, __LINE__))
    end if
