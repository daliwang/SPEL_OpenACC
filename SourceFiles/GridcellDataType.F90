module GridcellDataType
  ...
  !#py use ncdio_pio         , only : file_desc_t, ncd_double
  ...
  contains
    ...
    !#py procedure, public :: Restart => grc_ws_restart
...
contains
  ...
  subroutine grc_es_init(this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='GC_HEAT1',  units='J/m^2',  &
         !#py avgflag='A', long_name='initial gridcell total heat content', &
         !#py !#py ptr_lnd=this%heat1)

    this%heat2(begg:endg) = spval
    !#py call hist_addfld1d (fname='GC_HEAT2',  units='J/m^2',  &
         !#py avgflag='A', long_name='post land cover change total heat content', &
         !#py !#py ptr_lnd=this%heat2, default='inactive')

    this%liquid_water_temp1(begg:endg) = spval
    !#py call hist_addfld1d (fname='LIQUID_WATER_TEMP1', units='K', &
         !#py avgflag='A', long_name='initial gridcell weighted average liquid water temperature', &
         !#py !#py ptr_lnd=this%liquid_water_temp1, default='inactive')
  ...
  subroutine grc_ef_init(this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='EFLX_DYNBAL',  units='W/m^2',  &
         !#py avgflag='A', long_name='dynamic land cover change conversion energy flux', &
         !#py !#py ptr_lnd=this%eflx_dynbal)

  end subroutine grc_ef_init

  !------------------------------------------------------------------------
  subroutine grc_ef_restart(this, bounds,  flag)
    !
    ! !DESCRIPTION:
  ...
  subroutine grc_ws_init(this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='GC_LIQ1',  units='mm',  &
         !#py avgflag='A', long_name='initial gridcell total liq content', &
         !#py !#py ptr_lnd=this%liq1)

    this%liq2(begg:endg) = spval
    !#py call hist_addfld1d (fname='GC_LIQ2',  units='mm',  &
         !#py avgflag='A', long_name='post landuse change gridcell total liq content', &
         !#py !#py ptr_lnd=this%liq2, default='inactive')

    this%ice1(begg:endg) = spval
    !#py call hist_addfld1d (fname='GC_ICE1',  units='mm',  &
         !#py avgflag='A', long_name='initial gridcell total ice content', &
         !#py !#py ptr_lnd=this%ice1)

    this%ice2(begg:endg) = spval
    !#py call hist_addfld1d (fname='GC_ICE2',  units='mm',  &
         !#py avgflag='A', long_name='post land cover change total ice content', &
         !#py !#py ptr_lnd=this%ice2, default='inactive')

    this%tws(begg:endg) = spval
    !#py call hist_addfld1d (fname='TWS',  units='mm',  &
         !#py avgflag='A', long_name='total water storage', &
         !#py !#py ptr_lnd=this%tws)

    this%tws_month_beg(begg:endg) = spval
    !#py call hist_addfld1d (fname='TWS_MONTH_BEGIN',  units='mm',  &
         !#py avgflag='I', long_name='total water storage at the beginning of a month', &
         !#py !#py ptr_lnd=this%tws_month_beg)

    this%tws_month_end(begg:endg) = spval
    !#py call hist_addfld1d (fname='TWS_MONTH_END',  units='mm',  &
         !#py avgflag='I', long_name='total water storage at the end of a month', &
         !#py !#py ptr_lnd=this%tws_month_end)
  end subroutine grc_ws_init

  !------------------------------------------------------------------------
!#py   subroutine grc_ws_restart(this, bounds, ncid, flag)
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Read/Write gridcell water state information to/from restart file.
!#py     !
!#py     ! !USES:
!#py     !
!#py     ! !ARGUMENTS:
!#py     class(gridcell_water_state) :: this
...
!#py     !-----------------------------------------------------------------------
!#py     call restartvar(ncid=ncid, flag=flag, varname='TWS_MONTH_BEGIN', xtype=ncd_double,  &
!#py          dim1name='gridcell', &
!#py          long_name='surface watertotal water storage at the beginning of a month', units='mm', &
!#py           interpinic_flag='interp', readvar=readvar, data=this%tws_month_beg)
!#py 
!#py !#py !#py   end subroutine grc_ws_restart
  ...
  subroutine grc_wf_init(this, begg, endg, bounds)
    ...
    !#py call hist_addfld1d (fname='QFLX_LIQ_DYNBAL',  units='mm/s',  &
         !#py avgflag='A', long_name='liq dynamic land cover change conversion runoff flux', &
         !#py !#py ptr_lnd=this%qflx_liq_dynbal)

    this%qflx_ice_dynbal(begg:endg) = spval
    !#py call hist_addfld1d (fname='QFLX_ICE_DYNBAL',  units='mm/s',  &
         !#py avgflag='A', long_name='ice dynamic land cover change conversion runoff flux', &
         !#py !#py ptr_lnd=this%qflx_ice_dynbal)
  ...
  subroutine grc_cs_init(this, begg, endg,carbon_type)
    ...
    if (.not. use_fates) then
       if (carbon_type == 'c12') then
          ...
          !#py call hist_addfld1d (fname='SEEDC_GRC', units='gC/m^2', &
               !#py avgflag='A', long_name='pool for seeding new PFTs via dynamic landcover', &
               !#py !#py ptr_gcell=this%seedc)
       end if
       if (carbon_type == 'c13') then
          this%seedc(begg:endg) = spval
          !#py call hist_addfld1d (fname='C13_SEEDC_GRC', units='gC/m^2', &
               !#py avgflag='A', long_name='pool for seeding new PFTs via dynamic landcover', &
               !#py !#py ptr_gcell=this%seedc)
       end if
       if (carbon_type == 'c14') then
          this%seedc(begg:endg) = spval
          !#py call hist_addfld1d (fname='C14_SEEDC_GRC', units='gC/m^2', &
               !#py avgflag='A', long_name='pool for seeding new PFTs via dynamic landcover', &
               !#py !#py ptr_gcell=this%seedc)
       end if
    end if
  ...
  subroutine grc_cf_init(this, begg, endg, carbon_type)
    ...
    if (carbon_type == 'c12') then
       ...
       !#py call hist_addfld1d (fname='DWT_SEEDC_TO_LEAF_GRC', units='gC/m^2/s', &
            !#py avgflag='A', long_name='seed source to patch-level leaf', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_leaf, default='inactive')

       this%dwt_seedc_to_deadstem(begg:endg) = spval
       !#py call hist_addfld1d (fname='DWT_SEEDC_TO_DEADSTEM_GRC', units='gC/m^2/s', &
            !#py avgflag='A', long_name='seed source to patch-level deadstem', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_deadstem, default='inactive')

       this%dwt_conv_cflux(begg:endg) = spval
       !#py call hist_addfld1d (fname='DWT_CONV_CFLUX_GRC', units='gC/m^2/s', &
            !#py avgflag='A', &
            !#py long_name='conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)', &
            !#py !#py ptr_gcell=this%dwt_conv_cflux)

       this%dwt_conv_cflux_dribbled(begg:endg) = spval
       !#py call hist_addfld1d (fname='DWT_CONV_CFLUX_DRIBBLED', units='gC/m^2/s', &
            !#py avgflag='A', &
            !#py long_name='conversion C flux (immediate loss to atm), dribbled throughout the year', &
            !#py !#py ptr_gcell=this%dwt_conv_cflux_dribbled)

       this%dwt_prod10c_gain(begg:endg) = spval
       !#py call hist_addfld1d (fname='DWT_PROD10C_GAIN_GRC', units='gC/m^2/s', &
       ...
       !#py call hist_addfld1d (fname='DWT_PROD100C_GAIN_GRC', units='gC/m^2/s', &
       ...
       !#py call hist_addfld1d (fname='HRV_DEADSTEM_TO_PROD10C_GRC', units='gC/m^2/s', &
            !#py avgflag='A', long_name='dead stem harvest to 10-yr wood product pool', &
            !#py !#py ptr_col=this%hrv_deadstemc_to_prod10c, default='inactive')

       this%hrv_deadstemc_to_prod100c(begg:endg) = spval
       !#py call hist_addfld1d (fname='HRV_DEADSTEM_TO_PROD100C_GRC', units='gC/m^2/s', &
            !#py avgflag='A', long_name='dead stem harvest to 100-yr wood product pool', &
            !#py !#py ptr_col=this%hrv_deadstemc_to_prod100c, default='inactive')
    end if

    if ( carbon_type == 'c13' ) then

       this%dwt_seedc_to_leaf(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_SEEDC_TO_LEAF_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 seed source to patch-level leaf', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_leaf, default='inactive')

       this%dwt_seedc_to_deadstem(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_SEEDC_TO_DEADSTEM_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 seed source to patch-level deadstem', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_deadstem, default='inactive')

       this%dwt_conv_cflux(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_CONV_CFLUX_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', &
            !#py long_name='C13 conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)', &
            !#py !#py ptr_gcell=this%dwt_conv_cflux)

       this%dwt_conv_cflux_dribbled(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_CONV_CFLUX_DRIBBLED', units='gC13/m^2/s', &
            !#py avgflag='A', &
            !#py long_name='C13 conversion C flux (immediate loss to atm), dribbled throughout the year', &
            !#py !#py ptr_gcell=this%dwt_conv_cflux_dribbled)

       this%dwt_seedc_to_deadstem(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_SEEDC_TO_DEADSTEM', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 seed source to patch-level deadstem', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_deadstem, default='inactive')

       this%dwt_prod10c_gain(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_PROD10C_GAIN_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 landcover change-driven addition to 10-yr wood product pool', &
            !#py !#py ptr_col=this%dwt_prod10c_gain, default='inactive')

       this%dwt_prod100c_gain(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_DWT_PROD100C_GAIN_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 landcover change-driven addition to 100-yr wood product pool', &
            !#py !#py ptr_col=this%dwt_prod100c_gain, default='inactive')

       this%hrv_deadstemc_to_prod10c(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_HRV_DEADSTEM_TO_PROD10C_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 dead stem harvest to 10-yr wood product pool', &
            !#py !#py ptr_col=this%hrv_deadstemc_to_prod10c, default='inactive')

       this%hrv_deadstemc_to_prod100c(begg:endg) = spval
       !#py call hist_addfld1d (fname='C13_HRV_DEADSTEM_TO_PROD100C_GRC', units='gC13/m^2/s', &
            !#py avgflag='A', long_name='C13 dead stem harvest to 100-yr wood product pool', &
            !#py !#py ptr_col=this%hrv_deadstemc_to_prod100c, default='inactive')
    endif
    ...
    if (carbon_type == 'c14') then
       ...
       this%dwt_seedc_to_leaf(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_SEEDC_TO_LEAF_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 seed source to patch-level leaf', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_leaf, default='inactive')

       this%dwt_seedc_to_deadstem(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_SEEDC_TO_DEADSTEM_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 seed source to patch-level deadstem', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_deadstem, default='inactive')

       this%dwt_conv_cflux(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_CONV_CFLUX_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', &
            !#py long_name='C14 conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)', &
            !#py !#py ptr_gcell=this%dwt_conv_cflux)

       this%dwt_conv_cflux_dribbled(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_CONV_CFLUX_DRIBBLED', units='gC14/m^2/s', &
            !#py avgflag='A', &
            !#py long_name='C14 conversion C flux (immediate loss to atm), dribbled throughout the year', &
            !#py !#py ptr_gcell=this%dwt_conv_cflux_dribbled)

       this%dwt_seedc_to_deadstem(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_SEEDC_TO_DEADSTEM', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 seed source to patch-level deadstem', &
            !#py !#py ptr_gcell=this%dwt_seedc_to_deadstem, default='inactive')

       this%dwt_prod10c_gain(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_PROD10C_GAIN_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 landcover change-driven addition to 10-yr wood product pool', &
            !#py !#py ptr_col=this%dwt_prod10c_gain, default='inactive')

       this%dwt_prod100c_gain(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_DWT_PROD100C_GAIN_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 landcover change-driven addition to 100-yr wood product pool', &
            !#py !#py ptr_col=this%dwt_prod100c_gain, default='inactive')

       this%hrv_deadstemc_to_prod10c(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_HRV_DEADSTEM_TO_PROD10C_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 dead stem harvest to 10-yr wood product pool', &
            !#py !#py ptr_col=this%hrv_deadstemc_to_prod10c, default='inactive')

       this%hrv_deadstemc_to_prod100c(begg:endg) = spval
       !#py call hist_addfld1d (fname='C14_HRV_DEADSTEM_TO_PROD100C_GRC', units='gC14/m^2/s', &
            !#py avgflag='A', long_name='C14 dead stem harvest to 100-yr wood product pool', &
            !#py !#py ptr_col=this%hrv_deadstemc_to_prod100c, default='inactive')
    endif
  ...
  subroutine grc_ns_init(this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='SEEDN_GRC', units='gN/m^2', &
         !#py avgflag='A', long_name='pool for seeding new PFTs ', &
         !#py !#py ptr_gcell=this%seedn, default='inactive')
  ...
  subroutine grc_nf_init(this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='DWT_SEEDN_TO_LEAF_GRC', units='gN/m^2/s', &
         !#py avgflag='A', long_name='seed source to patch-level leaf', &
         !#py !#py ptr_gcell=this%dwt_seedn_to_leaf, default='inactive')

    this%dwt_seedn_to_deadstem(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_SEEDN_TO_DEADSTEM_GRC', units='gN/m^2/s', &
         !#py avgflag='A', long_name='seed source to patch-level deadstem', &
         !#py !#py ptr_gcell=this%dwt_seedn_to_deadstem, default='inactive')

    this%dwt_conv_nflux(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_CONV_NFLUX_GRC', units='gN/m^2/s', &
         !#py avgflag='A', &
         !#py long_name='conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)', &
         !#py !#py ptr_gcell=this%dwt_conv_nflux)

    this%dwt_seedn_to_npool(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_SEEDN_TO_NPOOL_GRC', units='gN/m^2/s', &
         !#py avgflag='A', long_name='seed source to PFT-level npool', &
         !#py !#py ptr_gcell=this%dwt_seedn_to_npool, default='inactive')

    this%dwt_prod10n_gain(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_PROD10N_GAIN_GRC', units='gN/m^2/s', &
         !#py avgflag='A', long_name='landcover change-driven addition to 10-yr wood product pool', &
         !#py !#py ptr_gcell=this%dwt_prod10n_gain, default='inactive')

    this%dwt_prod100n_gain(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_PROD100N_GAIN_GRC', units='gN/m^2/s', &
         !#py avgflag='A', long_name='landcover change-driven addition to 100-yr wood product pool', &
         !#py !#py ptr_gcell=this%dwt_prod100n_gain, default='inactive')
  ...
  subroutine grc_ps_init (this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='SEEDP_GRC', units='gP/m^2', &
         !#py avgflag='A', long_name='P pool for seeding new PFTs ', &
         !#py !#py ptr_gcell=this%seedp, default='inactive')
  ...
  subroutine grc_pf_init (this, begg, endg)
    ...
    !#py call hist_addfld1d (fname='DWT_SEEDP_TO_LEAF_GRC', units='gP/m^2/s', &
         !#py avgflag='A', long_name='seed source to patch-level leaf', &
         !#py !#py ptr_gcell=this%dwt_seedp_to_leaf, default='inactive')

    this%dwt_seedp_to_deadstem(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_SEEDP_TO_DEADSTEM_GRC', units='gP/m^2/s', &
         !#py avgflag='A', long_name='seed source to patch-level deadstem', &
         !#py !#py ptr_gcell=this%dwt_seedp_to_deadstem, default='inactive')

    this%dwt_conv_pflux(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_CONV_PFLUX_GRC', units='gP/m^2/s', &
         !#py avgflag='A', &
         !#py long_name='conversion C flux (immediate loss to atm) (0 at all times except first timestep of year)', &
         !#py !#py ptr_gcell=this%dwt_conv_pflux)

    this%dwt_seedp_to_ppool(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_SEEDP_TO_PPOOL_GRC', units='gP/m^2/s', &
         !#py avgflag='A', long_name='seed source to PFT-level', &
         !#py !#py ptr_gcell=this%dwt_seedp_to_ppool, default='inactive')

    this%dwt_prod10p_gain(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_PROD10P_GAIN_GRC', units='gP/m^2/s', &
         !#py avgflag='A', long_name='addition to 10-yr wood product pool', &
         !#py !#py ptr_gcell=this%dwt_prod10p_gain, default='inactive')

    this%dwt_prod100p_gain(begg:endg) = spval
    !#py call hist_addfld1d (fname='DWT_PROD100P_GAIN_GRC', units='gP/m^2/s', &
         !#py avgflag='A', long_name='addition to 10-yr wood product pool', &
         !#py !#py ptr_gcell=this%dwt_prod100p_gain, default='inactive')
