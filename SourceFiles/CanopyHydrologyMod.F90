module CanopyHydrologyMod
  ...
  !#py public :: CanopyHydrology_readnl ! Read namelist
...
contains
  ...
  !-----------------------------------------------------------------------
!#py   subroutine CanopyHydrology_readnl( NLFilename )
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Read the namelist for CanopyHydrology
!#py     !
!#py     ! !USES:
!#py     use spmdMod       , only : masterproc, mpicom
!#py     use fileutils     , only : getavu, relavu, opnfil
...
!#py     character(len=32) :: subname = 'CanopyHydrology_readnl'  ! subroutine name
!#py     !-----------------------------------------------------------------------
!#py 
!#py     namelist / elm_canopyhydrology_inparm / oldfflag
!#py 
!#py     ! ----------------------------------------------------------------------
...
!#py 
!#py        unitn = getavu()
!#py        write(iulog,*) 'Read in elm_canopyhydrology_inparm  namelist'
!#py        call opnfil (NLFilename, unitn, 'F')
!#py        call shr_nl_find_group_name(unitn, 'elm_canopyhydrology_inparm', status=ierr)
!#py        if (ierr == 0) then
!#py           read(unitn, elm_canopyhydrology_inparm, iostat=ierr)
!#py           if (ierr /= 0) then
!#py              call endrun(msg="ERROR reading elm_canopyhydrology_inparm namelist"//errmsg(__FILE__, __LINE__))
!#py           end if
!#py        end if
!#py        call relavu( unitn )
!#py 
!#py     end if
!#py     ! Broadcast namelist variables read in
!#py     call shr_mpi_bcast(oldfflag, mpicom)
!#py 
!#py    end subroutine CanopyHydrology_readnl
