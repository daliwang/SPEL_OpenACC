...
contains
  ...
  subroutine dyn_col_cs_Adjustments(proc_begc,proc_endc, nclumps, column_state_updater, col_cs)
    ...
    associate(&
     ...
     !$acc enter data create(adjustment_one_level(proc_begc:proc_endc,1:nlevdecomp,1:ndecomp_pools),sum1)
    ...
    !$acc loop gang vector independent private(begc,endc,bounds)
     ...
     !$acc exit data delete(adjustment_one_level(proc_begc:proc_endc,1:nlevdecomp,1:ndecomp_pools),sum1)
 ...
 !$acc parallel loop independent gang default(present)
  ...
  subroutine dyn_col_ps_Adjustments(proc_begc,proc_endc, nclumps,column_state_updater, col_ps)
    ...
    call cpu_time(startt) 
     ...
     call cpu_time(stopt)
    write(iulog,*) "col_ps_Adjustment::Reduction ",(stopt-startt)*1.E+3,"ms"
    ...
    call cpu_time(stopt)
    write(iulog,*) "col_ps_Adjustment::2D loop ",(stopt-startt)*1.E+3,"ms"
    ...
    call cpu_time(stopt)
   write(iulog,*) "col_ps_Adjustment::2D vr Reduction ",(stopt-startt)*1.E+3,"ms"
    ...
    !$acc loop gang vector independent private(begc,endc,bounds)
     ...
     !$acc exit data delete(adjustment_one_level(proc_begc:proc_endc,1:nlevdecomp,1:ndecomp_pools),sum1)
     write(iulog,*) "col_ps_Adjustment::Final parallel region ",(stopt-startt)*1.E+3,"ms"
