...
contains
  ...
  subroutine megan_factors_init( filename )
    ...
    !
    ! !USES:
    !#py use ncdio_pio, only : ncd_pio_openfile,ncd_inqdlen
    !#py use pio, only : pio_inq_varid,pio_get_var,file_desc_t,pio_closefile
    ...
    !#py call getfil(filename, locfn, 0)
    !#py call ncd_pio_openfile (ncid, trim(locfn), 0)

    !#py !#py call ncd_inqdlen( ncid, dimid, n_comps, name='Comp_Num')
    !#py !#py call ncd_inqdlen( ncid, dimid, n_classes, name='Class_Num')
    !#py !#py call ncd_inqdlen( ncid, dimid, n_patchs, name='PFT_Num')

    npfts = n_patchs

    !#py ierr = pio_inq_varid(ncid,'Class_EF', class_ef_vid)
    !#py ierr = pio_inq_varid(ncid,'Comp_EF',  comp_ef_vid)
    !#py ierr = pio_inq_varid(ncid,'Comp_Name',comp_name_vid)
    !#py ierr = pio_inq_varid(ncid,'Class_Num',class_num_vid)
    !#py ierr = pio_inq_varid(ncid,'Comp_MW',  comp_mw_vid)
    ...
    !#py ierr =  pio_get_var( ncid, comp_name_vid, comp_names ) 
    !#py ierr =  pio_get_var( ncid, comp_mw_vid, comp_molecwghts ) 
    !#py ierr =  pio_get_var( ncid, class_num_vid, class_nums )
    ...
    do i=1,n_comps
       ...
       !#py ierr = pio_get_var( ncid, comp_ef_vid,  start, count, comp_factors )
       start=(/class_nums(i),1/)
       !#py ierr = pio_get_var( ncid, class_ef_vid, start, count, class_factors  )
    ...
    !#py ierr = pio_inq_varid(ncid,'LDF', vid)
    !#py ierr = pio_get_var( ncid, vid, LDF ) 

    !#py ierr = pio_inq_varid(ncid,'Agro', vid)
    !#py ierr = pio_get_var( ncid, vid, Agro ) 

    !#py ierr = pio_inq_varid(ncid,'Amat', vid)
    !#py ierr = pio_get_var( ncid, vid, Amat ) 

    !#py ierr = pio_inq_varid(ncid,'Anew', vid)
    !#py ierr = pio_get_var( ncid, vid, Anew ) 

    !#py ierr = pio_inq_varid(ncid,'Aold', vid)
    !#py ierr = pio_get_var( ncid, vid, Aold ) 

    !#py ierr = pio_inq_varid(ncid,'betaT', vid)
    !#py ierr = pio_get_var( ncid, vid, betaT ) 

    !#py ierr = pio_inq_varid(ncid,'ct1', vid)
    !#py ierr = pio_get_var( ncid, vid, ct1 ) 

    !#py ierr = pio_inq_varid(ncid,'ct2', vid)
    !#py ierr = pio_get_var( ncid, vid, ct2 ) 

    !#py ierr = pio_inq_varid(ncid,'Ceo', vid)
    !#py ierr = pio_get_var( ncid, vid, Ceo ) 

    !#py call pio_closefile(ncid)
