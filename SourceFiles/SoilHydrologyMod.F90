...
contains
  ...
  subroutine SurfaceRunoff (bounds, num_hydrologyc, filter_hydrologyc, &
    ...
    associate(                                                        &
      ...
      !$acc xs(bounds%begc:bounds%endc), & 
      !$acc vol_ice(bounds%begc:bounds%endc,1:nlevgrnd),&
      !$acc fff(bounds%begc:bounds%endc), &
      !$acc A(bounds%begc:bounds%endc)            ,& 
      !$acc ex(bounds%begc:bounds%endc)           ,& 
      !$acc top_moist(bounds%begc:bounds%endc)    ,& 
      !$acc top_max_moist(bounds%begc:bounds%endc),& 
      !$acc top_ice(bounds%begc:bounds%endc)      & 
      !$acc )
   ...
   subroutine Infiltration(bounds, num_hydrologyc, filter_hydrologyc, num_urbanc, filter_urbanc, &
     ...
     associate(                                                                &
      ...
      !$acc   vol_ice(bounds%begc:bounds%endc,1:nlevgrnd),&
      !$acc   alpha_evap(bounds%begc:bounds%endc)        ,&
      !$acc   qflx_evap(bounds%begc:bounds%endc)         ,&
      !$acc   qflx_h2osfc_drain(bounds%begc:bounds%endc) ,&
      !$acc   qflx_in_h2osfc(bounds%begc:bounds%endc)    ,&
      !$acc   qflx_in_soil(bounds%begc:bounds%endc)      ,&
      !$acc   qflx_infl_excess(bounds%begc:bounds%endc)  )
   ...
   subroutine WaterTable(bounds, num_hydrologyc, filter_hydrologyc, num_urbanc, filter_urbanc, &
     ...
     associate(                                                            &
      ...
      !$acc enter data create( &
      !$acc   xs(bounds%begc:bounds%endc)             ,&
      !$acc   dzmm(bounds%begc:bounds%endc,1:nlevgrnd),&
      !$acc   jwt(bounds%begc:bounds%endc)            ,&
      !$acc   rsub_bot(bounds%begc:bounds%endc)       ,&
      !$acc   rsub_top(bounds%begc:bounds%endc)       ,&
      !$acc   fff(bounds%begc:bounds%endc)            ,&
      !$acc   xsi(bounds%begc:bounds%endc), fracice_rsub(bounds%begc:bounds%endc))
       !$acc parallel loop independent gang vector default(present)
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          nlevbed = nlev2bed(c)
      ...
      !$acc   rsub_top(bounds%begc:bounds%endc)       ,&
      !$acc   fff(bounds%begc:bounds%endc)            ,&
      !$acc   xsi(bounds%begc:bounds%endc)            ,&
      !$acc   fracice_rsub(bounds%begc:bounds%endc))
