Module DryDepVelocity
  ...
  !#py use clm_time_manager     , only : get_nstep, get_curr_date, get_curr_time
      ...
      pft_loop: do pi = bounds%begp,bounds%endp
         ...
         active: if (veg_pp%active(pi)) then
            ...
            if (wesveg == wveg_unset )then
               !#py write(iulog,*) 'elmveg = ', elmveg, 'lun_pp%itype = ', lun_pp%itype(l)
               !#py call endrun(decomp_index=pi, elmlevel=namep, &
