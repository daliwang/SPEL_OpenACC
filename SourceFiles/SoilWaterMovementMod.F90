module SoilWaterMovementMod
  ...
  !#py use elm_instMod , only : waterflux_vars, waterstate_vars, temperature_vars
...
contains
  ...
  !-----------------------------------------------------------------------
  !#pysubroutine init_soilwater_movement()
  !#py  !
  !#py  !DESCRIPTION
  !#py  !specify method for doing soil&root water interactions
  !#py  !
  !#py  use elm_varctl, only : use_vsfm, use_var_soil_thick, use_hydrstress
  !#py  !#py use spmdMod,    only : mpicom, MPI_LOGICAL
  !#py  !#py use shr_sys_mod,only : shr_sys_abort
  !#py  ! !ARGUMENTS:
  !#py  implicit none
  !#py  integer :: ier ! error status
  !#py  !------------------------------------------------------------------------------
  !#py  soilroot_water_method = zengdecker_2009
  !#py  zengdecker_2009_with_var_soil_thick = .false.
!#py
  !#py  ! GB-FIX-ME: The call to control_spmd() [in subroutine control_init()] before
  !#py  !            call to init_hydrology() would avoid the mpi broadcast
!#py
  !#py  call mpi_bcast (use_vsfm, 1, MPI_LOGICAL, 0, mpicom, ier)
  !#py  if (use_vsfm) soilroot_water_method = vsfm
!#py
  !#py  call mpi_bcast (use_var_soil_thick, 1, MPI_LOGICAL, 0, mpicom, ier)
  !#py  if (use_var_soil_thick .and. soilroot_water_method .eq. zengdecker_2009) then
  !#py     zengdecker_2009_with_var_soil_thick = .true.
  !#py  end if
!#py
  !#py  if (use_var_soil_thick .and. soilroot_water_method .ne. zengdecker_2009) then
  !#py     !#py call shr_sys_abort('ERROR: use_var_soil_thick not supported with anything but zengdecker_2009 at this time.')
  !#py  end if
!#py
  !#pyend subroutine init_soilwater_movement
  ...
  subroutine soilwater_zengdecker2009(bounds, num_hydrologyc, filter_hydrologyc, &
    ...
    associate(&
      ...
      !$acc  jtop(:),jbot(:), hk(:,:),dhkdw(:,:),amx(:,:),bmx(:,:),cmx(:,:),rmx(:,:),zmm(:,:),dzmm(:,:), &
      !$acc   dqidw0(bounds%begc:bounds%endc,1:nlevgrnd+1), dqidw1(bounds%begc:bounds%endc,1:nlevgrnd+1), &
      !$acc   dqodw1(bounds%begc:bounds%endc,1:nlevgrnd+1), dqodw2(bounds%begc:bounds%endc,1:nlevgrnd+1), &
      !$acc   dsmpdw(bounds%begc:bounds%endc,1:nlevgrnd+1), qin(bounds%begc:bounds%endc,1:nlevgrnd+1), & 
      !$acc   qout(bounds%begc:bounds%endc,1:nlevgrnd+1),smp(bounds%begc:bounds%endc,1:nlevgrnd), &
      !$acc   temp(bounds%begc:bounds%endc), jwt(bounds%begc:bounds%endc),dwat2(bounds%begc:bounds%endc,1:nlevgrnd+1),&
      !$acc    zimm(bounds%begc:bounds%endc,0:nlevgrnd), zq(bounds%begc:bounds%endc,1:nlevgrnd+1),vol_eq(bounds%begc:bounds%endc,1:nlevgrnd+1), &
      !$acc   zwtmm(bounds%begc:bounds%endc), imped(bounds%begc:bounds%endc,1:nlevgrnd), vol_ice(bounds%begc:bounds%endc,1:nlevgrnd), &
      !$acc   vwc_zwt(bounds%begc:bounds%endc), vwc_liq(bounds%begc:bounds%endc,1:nlevgrnd+1),smp_grad(bounds%begc:bounds%endc,1:nlevgrnd+1), sum1)
...
!#py    end subroutine Prepare_Data_for_EM_VSFM_Driver
   ...
   subroutine Compute_EffecRootFrac_And_VertTranSink(bounds, num_hydrologyc, &
      ...
      num_filterc = 0
      !$acc parallel loop independent gang vector default(present) private(c,fimperv) copy(num_filterc)
      do fc = 1, num_hydrologyc
         c = filter_hydrologyc(fc)
         l = col_pp%landunit(c)
         if ( (col_pp%itype(c) /= icol_road_perv) ) then
   ...
   subroutine Compute_EffecRootFrac_And_VertTranSink_Default(bounds, num_filterc, &
    ...
    associate(&
      ...
      !$acc enter data copyin(sum1,temp(:))

      !$acc parallel loop independent gang worker default(present) 
      do fc = 1, num_filterc
         c = filterc(fc)
         nlevbed = nlev2bed(c)
         !$acc loop worker private(sum1)
         do j = 1,nlevbed
            sum1 = 0._r8; 
            !$acc loop vector reduction(+:sum1)
      ...
      do j = 1,nlevsoi
         do fc = 1, num_filterc
            ...
            if(rootr_col(c,j)==0._r8)then
               ...
               do p = col_pp%pfti(c), col_pp%pftf(c)
                  if (veg_pp%active(p)) then
                      sum1 = sum1*(1._r8/(temp(fc)*rootr_col(c,j)))
                  end if
               end do 
               qflx_rootsoi_frac_patch(p,j) = sum1 
            end if 
         end do
      end do

      !$acc exit data delete(sum1,temp(:))
