module SoilLittDecompMod
  ...
  ! !PUBLIC MEMBER FUNCTIONS:
   ...
   !#py public :: readSoilLittDecompParams
...
contains
  ...
  !-----------------------------------------------------------------------
!#py    subroutine readSoilLittDecompParams ( ncid )
!#py      !
!#py      ! !DESCRIPTION:
!#py      ! Read parameters
!#py      !
...
!#py      !
!#py      ! !LOCAL VARIABLES:
!#py      character(len=32)  :: subname = 'CNDecompParamsType'
...
!#py      !-----------------------------------------------------------------------
!#py      allocate(CNDecompParamsInst%dnp)
!#py      tString='dnp'
...
!#py      CNDecompParamsInst%dnp=tempr
!#py 
!#py    end subroutine readSoilLittDecompParams
...
!                           ratio = cp_decomp_pools(c,j,cascade_receiver_pool(k))/cp_decomp_pools(c,j,k_donor_pool)
      ...
      do k = 1, ndecomp_cascade_transitions
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
               phr_vr(c,j) = phr_vr(c,j) + rf_decomp_cascade(c,j,k) * p_decomp_cpool_loss(fc,j,k)
            end do
         end do
      end do
      ...
      if (use_lch4) then
         ...
         do k = 1, ndecomp_cascade_transitions
            do j = 1,nlevdecomp
               do fc = 1,num_soilc
                  c = filter_soilc(fc)
                  hrsum(fc,j) = hrsum(fc,j) + rf_decomp_cascade(c,j,k) * p_decomp_cpool_loss(fc,j,k)
               end do
            end do
         end do
