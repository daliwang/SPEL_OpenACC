...
! plant hydraulics
  ...
  subroutine InitHistory(this, bounds)
    ...
    !#py call hist_addfld1d (fname='RH_LEAF', units='fraction', &
         !#py avgflag='A', long_name='fractional humidity at leaf surface', &
         !#py !#py ptr_patch=this%rh_leaf_patch, set_spec=spval, default='inactive')

    this%fpsn_patch(begp:endp) = spval
    !#py call hist_addfld1d (fname='FPSN', units='umol/m2s',  &
         !#py avgflag='A', long_name='photosynthesis', &
         !#py !#py ptr_patch=this%fpsn_patch, set_lake=0._r8, set_urb=0._r8)

    this%fpsn_wc_patch(begp:endp) = spval
    !#py call hist_addfld1d (fname='FPSN_WC', units='umol/m2s',  &
         !#py avgflag='A', long_name='Rubisco-limited photosynthesis', &
         !#py !#py ptr_patch=this%fpsn_wc_patch, set_lake=0._r8, set_urb=0._r8)

    this%fpsn_wj_patch(begp:endp) = spval
    !#py call hist_addfld1d (fname='FPSN_WJ', units='umol/m2s',  &
         !#py avgflag='A', long_name='RuBP-limited photosynthesis', &
         !#py !#py ptr_patch=this%fpsn_wj_patch, set_lake=0._r8, set_urb=0._r8)

    this%fpsn_wp_patch(begp:endp) = spval
    !#py call hist_addfld1d (fname='FPSN_WP', units='umol/m2s',  &
         !#py avgflag='A', long_name='Product-limited photosynthesis', &
         !#py !#py ptr_patch=this%fpsn_wp_patch, set_lake=0._r8, set_urb=0._r8)

    if (use_cn) then
       this%psnsun_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='PSNSUN', units='umolCO2/m^2/s', &
            !#py avgflag='A', long_name='sunlit leaf photosynthesis', &
            !#py !#py ptr_patch=this%psnsun_patch)

       this%psnsha_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='PSNSHA', units='umolCO2/m^2/s', &
            !#py avgflag='A', long_name='shaded leaf photosynthesis', &
            !#py !#py ptr_patch=this%psnsha_patch)
    end if

    if ( use_c13 ) then
       this%c13_psnsun_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='C13_PSNSUN', units='umolCO2/m^2/s', &
            !#py avgflag='A', long_name='C13 sunlit leaf photosynthesis', &
            !#py !#py ptr_patch=this%c13_psnsun_patch)

       this%c13_psnsha_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='C13_PSNSHA', units='umolCO2/m^2/s', &
            !#py avgflag='A', long_name='C13 shaded leaf photosynthesis', &
            !#py !#py ptr_patch=this%c13_psnsha_patch)
    end if

    if ( use_c14 ) then
       this%c14_psnsun_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='C14_PSNSUN', units='umolCO2/m^2/s', &
            !#py avgflag='A', long_name='C14 sunlit leaf photosynthesis', &
            !#py !#py ptr_patch=this%c14_psnsun_patch)

       this%c14_psnsha_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='C14_PSNSHA', units='umolCO2/m^2/s', &
            !#py avgflag='A', long_name='C14 shaded leaf photosynthesis', &
            !#py !#py ptr_patch=this%c14_psnsha_patch)
    end if

    if ( use_c13 ) then
       this%rc13_canair_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='RC13_CANAIR', units='proportion', &
            !#py avgflag='A', long_name='C13/C(12+13) for canopy air', &
            !#py !#py ptr_patch=this%rc13_canair_patch)

       this%rc13_psnsun_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='RC13_PSNSUN', units='proportion', &
            !#py avgflag='A', long_name='C13/C(12+13) for sunlit photosynthesis', &
            !#py !#py ptr_patch=this%rc13_psnsun_patch)

       this%rc13_psnsha_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='RC13_PSNSHA', units='proportion', &
            !#py avgflag='A', long_name='C13/C(12+13) for shaded photosynthesis', &
            !#py !#py ptr_patch=this%rc13_psnsha_patch)
    endif
    ...
    if ( use_c13 ) then
       ...
       !#py call hist_addfld1d (fname='ALPHAPSNSUN', units='proportion', &
            !#py avgflag='A', long_name='sunlit c13 fractionation', &
            !#py !#py ptr_patch=this%alphapsnsun_patch, default='inactive')

       this%alphapsnsha_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='ALPHAPSNSHA', units='proportion', &
            !#py avgflag='A', long_name='shaded c13 fractionation', &
            !#py !#py ptr_patch=this%alphapsnsha_patch, default='inactive')
    endif

    this%rssun_patch(begp:endp) = spval
    !#py call hist_addfld1d (fname='RSSUN', units='s/m',  &
         !#py avgflag='M', long_name='sunlit leaf stomatal resistance', &
         !#py !#py ptr_patch=this%rssun_patch, set_lake=spval, set_urb=spval, default='inactive')

    this%rssha_patch(begp:endp) = spval
    !#py call hist_addfld1d (fname='RSSHA', units='s/m',  &
         !#py avgflag='M', long_name='shaded leaf stomatal resistance', &
         !#py !#py ptr_patch=this%rssha_patch, set_lake=spval, set_urb=spval, default='inactive')
...
!#py     !-----------------------------------------------------------------------
!#py     if ( use_c13 ) then
!#py        call restartvar(ncid=ncid, flag=flag, varname='rc13_canair', xtype=ncd_double,  &
!#py             dim1name='pft', long_name='', units='', &
!#py             interpinic_flag='interp', readvar=readvar, data=this%rc13_canair_patch)
!#py 
!#py        call restartvar(ncid=ncid, flag=flag, varname='rc13_psnsun', xtype=ncd_double,  &
!#py             dim1name='pft', long_name='', units='', &
!#py             interpinic_flag='interp', readvar=readvar, data=this%rc13_psnsun_patch)
!#py 
!#py        call restartvar(ncid=ncid, flag=flag, varname='rc13_psnsha', xtype=ncd_double,  &
!#py             dim1name='pft', long_name='', units='', &
!#py             interpinic_flag='interp', readvar=readvar, data=this%rc13_psnsha_patch)
!#py     endif
!#py 
!#py !#py !#py   end subroutine Restart
  ...
  subroutine TimeStepInit (this, bounds)
    ...
    do p = bounds%begp, bounds%endp
       ...
       !#py if (lun_pp%itype(l) == istsoil .or. lun_pp%itype(l) == istcrop &
            !#py .or. lun_pp%itype(l) == istice .or. lun_pp%itype(l) == istice_mec &
            !#py .or. lun_pp%itype(l) == istwet) then
          if (use_c13) then
