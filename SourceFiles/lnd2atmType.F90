...
contains
  ...
  subroutine InitHistory(this, bounds)
    ...
    !#py call hist_addfld1d (fname='FSH', units='W/m^2',  &
         !#py avgflag='A', long_name='sensible heat', &
         !#py !#py ptr_lnd=this%eflx_sh_tot_grc)
       
    this%qflx_rofliq_grc(begg:endg) = 0._r8
    !#py call hist_addfld1d (fname='QRUNOFF',  units='mm/s',  &
         !#py avgflag='A', long_name='total liquid runoff (does not include QSNWCPICE)', &
         !#py !#py ptr_lnd=this%qflx_rofliq_grc)

    this%qflx_rofice_grc(begg:endg) = 0._r8
    !#py call hist_addfld1d (fname='QSNWCPICE',  units='mm/s',  &
         !#py avgflag='A', long_name='excess snowfall due to snow capping', &
         !#py !#py ptr_lnd=this%qflx_rofice_grc)

    if (use_lch4) then
       this%flux_ch4_grc(begg:endg) = 0._r8
       !#py call hist_addfld1d (fname='FCH4', units='kgC/m2/s', &
            !#py avgflag='A', long_name='Gridcell surface CH4 flux to atmosphere (+ to atm)', &
            !#py !#py ptr_lnd=this%flux_ch4_grc)

       this%nem_grc(begg:endg) = spval
       !#py call hist_addfld1d (fname='NEM', units='gC/m2/s', &
            !#py avgflag='A', long_name='Gridcell net adjustment to NEE passed to atm. for methane production', &
            !#py !#py ptr_lnd=this%nem_grc)
    end if
