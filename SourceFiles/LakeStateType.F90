...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    subroutine InitHistory(this, bounds)
      ...
      !#py call hist_addfld2d (fname='LAKEICEFRAC',  units='unitless', type2d='levlak', &
           !#py avgflag='A', long_name='lake layer ice mass fraction', &
           !#py !#py ptr_col=this%lake_icefrac_col)

      this%lake_icethick_col(begc:endc) = spval ! This will be more useful than LAKEICEFRAC for many users.
      !#py call hist_addfld1d (fname='LAKEICETHICK', units='m', &
           !#py avgflag='A', long_name='thickness of lake ice (including physical expansion on freezing)', &
           !#py !#py ptr_col=this%lake_icethick_col, set_nolake=spval)

      this%savedtke1_col(begc:endc) = spval
      !#py call hist_addfld1d (fname='TKE1',  units='W/(mK)', &
           !#py avgflag='A', long_name='top lake level eddy thermal conductivity', &
           !#py !#py ptr_col=this%savedtke1_col)

      this%ram1_lake_patch(begp:endp) = spval
      !#py call hist_addfld1d (fname='RAM_LAKE', units='s/m', &
           !#py avgflag='A', long_name='aerodynamic resistance for momentum (lakes only)', &
           !#py !#py ptr_patch=this%ram1_lake_patch, set_nolake=spval, default='inactive')

      this%ust_lake_col(begc:endc) = spval
      !#py call hist_addfld1d (fname='UST_LAKE', units='m/s', &
           !#py avgflag='A', long_name='friction velocity (lakes only)', &
           !#py !#py ptr_col=this%ust_lake_col, set_nolake=spval, default='inactive')
    ...
    subroutine InitCold(this, bounds)
      ...
      !#py !#py call ncd_io(ncid=ncid, varname='ETALAKE', flag='read', data=etal_in, dim1name=grlnd, readvar=readvar)
      if (.not. readvar) then
         if (masterproc) then
            !#py write(iulog,*) 'WARNING:: ETALAKE not found on surface data set. All lake columns will have eta', &
                 !#py ' set equal to default value as a function of depth.'
         end if
         etal_in(:) = -1._r8
      end if
      ...
      !#py !#py call ncd_io(ncid=ncid, varname='LAKEFETCH', flag='read', data=lakefetch_in, dim1name=grlnd, readvar=readvar)
      if (.not. readvar) then
         if (masterproc) then
            !#py write(iulog,*) 'WARNING:: LAKEFETCH not found on surface data set. All lake columns will have fetch', &
                 !#py ' set equal to default value as a function of depth.'
         end if
         lakefetch_in(:) = -1._r8
      end if
...
!#py       !-----------------------------------------------------------------------
!#py 
!#py       call restartvar(ncid=ncid, flag=flag, varname='LAKE_ICEFRAC', xtype=ncd_double,  &
!#py            dim1name='column', dim2name='levlak', switchdim=.true., &
!#py            long_name='lake layer ice fraction', units='kg/kg', &
!#py            interpinic_flag='interp', readvar=readvar, data=this%lake_icefrac_col)
!#py 
!#py       call restartvar(ncid=ncid, flag=flag, varname='SAVEDTKE1', xtype=ncd_double,  &
!#py            dim1name='column', &
!#py            long_name='top lake layer eddy conductivity', units='W/(m K)', &
!#py            interpinic_flag='interp', readvar=readvar, data=this%savedtke1_col)
!#py 
!#py       call restartvar(ncid=ncid, flag=flag, varname='USTLAKE', xtype=ncd_double,  &
!#py            dim1name='column', &
!#py            long_name='friction velocity for lakes', units='m/s', &
!#py            interpinic_flag='interp', readvar=readvar, data=this%ust_lake_col)
!#py 
