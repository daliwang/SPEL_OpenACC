module VOCEmissionMod
  ...
  !#py use shr_megan_mod      , only : shr_megan_megcomps_n, shr_megan_megcomp_t, shr_megan_linkedlist
  !#py use shr_megan_mod      , only : shr_megan_mechcomps_n, shr_megan_mechcomps, shr_megan_mapped_emisfctrs
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    !#py use shr_megan_mod   , only : shr_megan_factors_file
    ...
    !#py meg_cmp => shr_megan_linkedlist
  ...
  subroutine InitHistory(this, bounds)
    ...
    if (shr_megan_megcomps_n>0) then
       ...
       do while(associated(meg_cmp))
          ...
          !#py call hist_addfld1d ( fname='MEG_'//trim(meg_cmp%name), units='kg/m2/sec',  &
               !#py avgflag='A', long_name='MEGAN flux', &
               !#py !#py ptr_patch=meg_out(imeg)%flux_out, set_lake=0._r8, set_urb=0._r8 )

          meg_cmp => meg_cmp%next_megcomp
       enddo
       
       this%vocflx_tot_patch(begp:endp)= spval
       !#py call hist_addfld1d (fname='VOCFLXT', units='moles/m2/sec',  &
            !#py avgflag='A', long_name='total VOC flux into atmosphere', &
            !#py !#py ptr_patch=this%vocflx_tot_patch, set_lake=0._r8, set_urb=0._r8)

       this%gamma_out_patch(begp:endp)   = spval
       !#py call hist_addfld1d (fname='GAMMA', units='non',  &
            !#py avgflag='A', long_name='total gamma for VOC calc', &
            !#py !#py ptr_patch=this%gamma_out_patch, set_lake=0._r8, default='inactive')

       this%gammaL_out_patch(begp:endp)  = spval
       !#py call hist_addfld1d (fname='GAMMAL', units='non',  &
            !#py avgflag='A', long_name='gamma L for VOC calc', &
            !#py !#py ptr_patch=this%gammaL_out_patch, set_lake=0._r8, default='inactive')

       this%gammaT_out_patch(begp:endp)  = spval
       !#py call hist_addfld1d (fname='GAMMAT', units='non',  &
            !#py avgflag='A', long_name='gamma T for VOC calc', &
            !#py !#py ptr_patch=this%gammaT_out_patch, set_lake=0._r8, default='inactive')

       this%gammaP_out_patch(begp:endp)  = spval
       !#py call hist_addfld1d (fname='GAMMAP', units='non',  &
            !#py avgflag='A', long_name='gamma P for VOC calc', &
            !#py !#py ptr_patch=this%gammaP_out_patch, set_lake=0._r8, default='inactive')

       this%gammaA_out_patch(begp:endp)  = spval
       !#py call hist_addfld1d (fname='GAMMAA', units='non',  &
            !#py avgflag='A', long_name='gamma A for VOC calc', &
            !#py !#py ptr_patch=this%gammaA_out_patch, set_lake=0._r8, default='inactive')

       this%gammaS_out_patch(begp:endp)  = spval
       !#py call hist_addfld1d (fname='GAMMAS', units='non',  &
            !#py avgflag='A', long_name='gamma S for VOC calc', &
            !#py !#py ptr_patch=this%gammaS_out_patch, set_lake=0._r8, default='inactive')

       this%gammaC_out_patch(begp:endp)  = spval
       !#py call hist_addfld1d (fname='GAMMAC', units='non',  &
            !#py avgflag='A', long_name='gamma C for VOC calc', &
            !#py !#py ptr_patch=this%gammaC_out_patch, set_lake=0._r8, default='inactive')

       this%EOPT_out_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='EOPT', units='non',  &
            !#py avgflag='A', long_name='Eopt coefficient for VOC calc', &
            !#py !#py ptr_patch=this%Eopt_out_patch, set_lake=0._r8, default='inactive')

       this%topt_out_patch(begp:endp)    = spval
       !#py call hist_addfld1d (fname='TOPT', units='non',  &
            !#py avgflag='A', long_name='topt coefficient for VOC calc', &
            !#py !#py ptr_patch=this%topt_out_patch, set_lake=0._r8, default='inactive')

       this%alpha_out_patch(begp:endp)    = spval
       !#py call hist_addfld1d (fname='ALPHA', units='non',  &
            !#py avgflag='A', long_name='alpha coefficient for VOC calc', &
            !#py !#py ptr_patch=this%alpha_out_patch, set_lake=0._r8, default='inactive')

       this%cp_out_patch(begp:endp)      = spval   
       !#py call hist_addfld1d (fname='currentPatch', units='non',  &
            !#py avgflag='A', long_name='currentPatch coefficient for VOC calc', &
            !#py !#py ptr_patch=this%cp_out_patch, set_lake=0._r8, default='inactive')

       this%paru_out_patch(begp:endp)    = spval   
       !#py call hist_addfld1d (fname='PAR_sun', units='umol/m2/s', &
            !#py avgflag='A', long_name='sunlit PAR', &
            !#py !#py ptr_patch=this%paru_out_patch, set_lake=0._r8, default='inactive')

       this%par24u_out_patch(begp:endp)  = spval 
       !#py call hist_addfld1d (fname='PAR24_sun', units='umol/m2/s', &
            !#py avgflag='A', long_name='sunlit PAR (24 hrs)', &
            !#py !#py ptr_patch=this%par24u_out_patch, set_lake=0._r8, default='inactive')

       this%par240u_out_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='PAR240_sun', units='umol/m2/s', &
            !#py avgflag='A', long_name='sunlit PAR (240 hrs)', &
            !#py !#py ptr_patch=this%par240u_out_patch, set_lake=0._r8, default='inactive')

       this%para_out_patch(begp:endp)    = spval   
       !#py call hist_addfld1d (fname='PAR_shade', units='umol/m2/s', &
            !#py avgflag='A', long_name='shade PAR', &
            !#py !#py ptr_patch=this%para_out_patch, set_lake=0._r8, default='inactive')

       this%par24a_out_patch(begp:endp)  = spval 
       !#py call hist_addfld1d (fname='PAR24_shade', units='umol/m2/s', &
            !#py avgflag='A', long_name='shade PAR (24 hrs)', &
            !#py !#py ptr_patch=this%par24a_out_patch, set_lake=0._r8, default='inactive')

       this%par240a_out_patch(begp:endp) = spval
       !#py call hist_addfld1d (fname='PAR240_shade', units='umol/m2/s', &
            !#py avgflag='A', long_name='shade PAR (240 hrs)', &
            !#py !#py ptr_patch=this%par240a_out_patch, set_lake=0._r8, default='inactive')
  ...
  subroutine InitCold(this, bounds)
    ...
    if (.not. readvar) then
       !#py !#py call endrun(msg='iniTimeConst: errror reading EF1_BTR'//errMsg(__FILE__, __LINE__))
    end if
    ...
    if (.not. readvar) then
       !#py !#py call endrun(msg='iniTimeConst: errror reading EF1_FET'//errMsg(__FILE__, __LINE__))
    end if
    ...
    if (.not. readvar) then
       !#py !#py call endrun(msg='iniTimeConst: errror reading EF1_FDT'//errMsg(__FILE__, __LINE__))
    end if
    ...
    if (.not. readvar) then
       !#py !#py call endrun(msg='iniTimeConst: errror reading EF1_SHR'//errMsg(__FILE__, __LINE__))
    end if
    ...
    if (.not. readvar) then
       !#py !#py call endrun(msg='iniTimeConst: errror reading EF1_GRS'//errMsg(__FILE__, __LINE__))
    end if
    ...
    if (.not. readvar) then
       !#py !#py call endrun(msg='iniTimeConst: errror reading EF1_CRP'//errMsg(__FILE__, __LINE__))
    end if
  ...
  subroutine VOCEmission (bounds, num_soilp, filter_soilp, &
    ...
    if ( nlevcan /= 1 )then
       !#py !#py call endrun( subname//' error: can NOT work without nlevcan == 1' )
    end if
    ...
    !#py do imeg=1,shr_megan_megcomps_n
    ...
    do fp = 1,num_soilp
       ...
       if (veg_pp%itype(p) > 0) then 
          ...
          meg_cmp_loop: do while(associated(meg_cmp))
             ...
             !#py if ( trim(meg_cmp%name) == 'isoprene' .and. shr_megan_mapped_emisfctrs) then
             ...
             if (debug .and. gamma > 0.0_r8) then
                !#py write(iulog,*) 'MEGAN: n, megan name, epsilon, gamma, vocflx: ', &
                     !#py imeg, meg_cmp%name, epsilon, gamma, vocflx_meg(imeg), gamma_p,gamma_t,gamma_a,gamma_sm,gamma_l
             endif
          ...
          !#py do imech = 1,shr_megan_mechcomps_n
             !#py n_meg_comps = shr_megan_mechcomps(imech)%n_megan_comps
             do imeg = 1,n_meg_comps ! loop over number of megan compounds that make up the nth mechanism compoud
                !#py ii = shr_megan_mechcomps(imech)%megan_comps(imeg)%ptr%index
  ...
  function get_gamma_SM(clayfrac_in, sandfrac_in, h2osoi_vol_in, h2osoi_ice_in, dz_in, &
    ...
    if ((clayfrac_in > 0) .and. (sandfrac_in > 0)) then 
       ...
       if (get_gamma_SM > 1.0_r8) then 
          !#py write(iulog,*) 'healdSM > 1: gamma_SM, nl', get_gamma_SM, nl
          get_gamma_SM=1.0_r8
       endif
