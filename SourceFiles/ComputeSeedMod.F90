...
contains
  ...
  subroutine ComputeSeedAmounts(p,                          &
    ...
    if (present(pool_seed_patch)) then
       if (.not. present(pool_seed_param)) then
          !#py call endrun( ': pool_seed_patch can only be provided with pool_seed_param')
       end if
    end if
#endif
  ...
  function SpeciesTypeMultiplier(species, pft_type, component) result(multiplier)
    ...
    case (CN_SPECIES_N)
       ...
       case default
#ifndef _OPENACC
          !#py write(iulog,*) subname//' ERROR: unknown component: ', component 
          !#py call endrun(subname//' ERROR: unknown component')
#endif 
       end select
    ...
    case default
#ifndef _OPENACC
          !#py write(iulog,*) subname//' ERROR: unknown species: ', species 
          !#py call endrun(subname//' ERROR: unknown species')
#endif 
    end select
