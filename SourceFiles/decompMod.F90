module decompMod
  ...
  interface get_clump_bounds
     ...
     module procedure get_clump_bounds_gpu
  end interface
  ...
  ! Created pared down version of bounds_type for gpu.
  type, public :: bounds_type_gpu
     ! The following variables correspond to "Local" quantities
     integer :: begg, endg                       ! beginning and ending gridcell index
     integer :: begt, endt                       ! beginning and ending topographic unit index
     integer :: begl, endl                       ! beginning and ending landunit index
...
contains
  ...
  !------------------------------------------------------------------------------
   subroutine get_clump_bounds_gpu (n, bounds)
     ...
     type(bounds_type_gpu), intent(out) :: bounds ! clump bounds
     !
     ! !LOCAL VARIABLES:
  ...
  !------------------------------------------------------------------------------
   subroutine get_clump_bounds_new (n, bounds)
     ...
     if ( OMP_GET_NUM_THREADS() == 1 .and. OMP_GET_MAX_THREADS() > 1 )then
        !#py call shr_sys_abort( trim(subname)//' ERROR: Calling from inside a non-threaded region)')
     end if
#endif
   ...
   subroutine get_proc_bounds_new (bounds)
     ...
     if ( OMP_GET_NUM_THREADS() > 1 )then
        !#py call shr_sys_abort( trim(subname)//' ERROR: Calling from inside  a threaded region')
     end if
#endif
