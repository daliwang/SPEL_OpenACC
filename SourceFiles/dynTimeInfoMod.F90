...
contains
  ...
  subroutine set_current_year(this)
    ...
    case default
       !#py write(iulog,*) subname, ': unknown year position: ', this%year_position%flag
  ...
  subroutine set_current_year_from_year(this, cur_year)
    ...
    if (year_in_current_interval(this, cur_year)) then
       ...
       if (cur_year < years(1)) then
          ...
          if (.not. found) then
             !#py write(iulog,*) subname//' ERROR: model year not found in pftdyn timeseries'
             !#py write(iulog,*)'model year = ',cur_year
             !#py !#py call endrun(msg=errMsg(__FILE__, __LINE__))
          end if
       end if
    end if
