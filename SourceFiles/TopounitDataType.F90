module TopounitDataType
  ...
  contains
    ...
    procedure, public :: InitAccVars   => init_acc_vars_top_as
     !#py procedure, public :: UpdateAccVars => update_acc_vars_top_as
  ...
  contains
    ...
    procedure, public :: InitAccVars   => init_acc_vars_top_af
     !#py procedure, public :: UpdateAccVars => update_acc_vars_top_af
  ...
  contains
    ...
    !#py procedure, public :: Restart => restart_top_es
  ...
  subroutine init_top_as(this, begt, endt)
    ...
    !#py call hist_addfld1d (fname='TBOT', units='K',  &
         !#py avgflag='A', long_name='atmospheric air temperature', &
         !#py !#py ptr_topo=this%tbot,t2g_scale_type='unity')

    this%thbot(begt:endt) = spval
    !#py call hist_addfld1d (fname='THBOT', units='K',  &
         !#py avgflag='A', long_name='atmospheric air potential temperature', &
         !#py !#py ptr_topo=this%thbot,t2g_scale_type='unity')

    this%pbot(begt:endt) = spval
    !#py call hist_addfld1d (fname='PBOT', units='Pa',  &
         !#py avgflag='A', long_name='atmospheric pressure', &
         !#py !#py ptr_topo=this%pbot,t2g_scale_type='unity')

    this%qbot(begt:endt) = spval
    !#py call hist_addfld1d (fname='QBOT', units='kg/kg',  &
         !#py avgflag='A', long_name='atmospheric specific humidity', &
         !#py !#py ptr_topo=this%qbot,t2g_scale_type='unity')

    this%rhbot(begt:endt) = spval
     !#py call hist_addfld1d (fname='RH', units='%',  &
          !#py avgflag='A', long_name='atmospheric relative humidity', &
           !#py !#py ptr_gcell=this%rhbot, default='inactive')

    this%windbot(begt:endt) = spval
    !#py call hist_addfld1d (fname='WIND', units='m/s',  &
         !#py avgflag='A', long_name='atmospheric wind velocity magnitude', &
         !#py !#py ptr_topo=this%windbot,t2g_scale_type='unity')

    this%zbot(begt:endt) = spval
    !#py call hist_addfld1d (fname='ZBOT', units='m',  &
         !#py avgflag='A', long_name='atmospheric reference height', &
         !#py !#py ptr_topo=this%zbot,t2g_scale_type='unity')

    this%pco2bot(begt:endt) = spval
    !#py call hist_addfld1d (fname='PCO2', units='Pa',  &
         !#py avgflag='A', long_name='atmospheric partial pressure of CO2', &
         !#py !#py ptr_topo=this%pco2bot,t2g_scale_type='unity')

    if (use_lch4) then
       this%pch4bot(begt:endt) = spval
       !#py call hist_addfld1d (fname='PCH4', units='Pa',  &
            !#py avgflag='A', long_name='atmospheric partial pressure of CH4', &
            !#py !#py ptr_topo=this%pch4bot,t2g_scale_type='unity')
    end if
  ...
  subroutine init_acc_buffer_top_as (this, bounds)
    ...
    if (use_fates) then
        !#py call init_accum_field (name='RH24H', units='%', &
             !#py desc='24hr running mean of relative humidity', accum_type='runmean', accum_period=-1, &
              !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)
        !#py call init_accum_field (name='WIND24H', units='m/s', &
             !#py desc='24hr running mean of wind', accum_type='runmean', accum_period=-1, &
              !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)
    end if
  ...
  subroutine init_acc_vars_top_as(this, bounds)
    ...
    if (ier/=0) then
        !#py write(iulog,*)' in '
        !#py call endrun(msg="extract_accum_hist allocation error for rbufslt"//&
    ...
    if (use_fates) then
        !#py call extract_accum_field ('RH24H', rbufslt, nstep)
       this%rh24h(begt:endt) = rbufslt(begt:endt)

        !#py call extract_accum_field ('WIND24H', rbufslt, nstep)
  ...
  !-----------------------------------------------------------------------
!#py    subroutine update_acc_vars_top_as (this, bounds)
!#py      !
!#py      ! USES
...
!#py      !
!#py      ! !ARGUMENTS:
!#py      class(topounit_atmospheric_state)    :: this
!#py      type(bounds_type)      , intent(in) :: bounds
!#py      !
!#py      ! !LOCAL VARIABLES:
!#py      integer :: g,t,c,p                   ! indices
!#py      integer :: dtime                     ! timestep size [seconds]
!#py      integer :: nstep                     ! timestep number
!#py      integer :: ier                       ! error status
!#py      integer :: begt, endt
!#py      real(r8), pointer :: rbufslt(:)      ! temporary single level - topounit level
!#py      !---------------------------------------------------------------------
!#py 
!#py      begt = bounds%begt; endt = bounds%endt
!#py 
!#py       !#py nstep = get_nstep()
!#py 
!#py      ! Allocate needed dynamic memory for single level topounit field
!#py 
!#py      allocate(rbufslt(begt:endt), stat=ier)
!#py      if (ier/=0) then
!#py          !#py write(iulog,*)'update_accum_hist allocation error for rbufslt'
!#py           !#py !#py call endrun(msg=errMsg(__FILE__, __LINE__))
!#py      endif
!#py 
!#py      ! Accumulate 24-hour running mean of relative humidity
!#py      do t = begt,endt
!#py         rbufslt(t) = this%rhbot(t)
!#py      end do
!#py      if (use_fates) then
!#py         call update_accum_field  ('RH24H', rbufslt, nstep)
!#py         call extract_accum_field ('RH24H', this%rh24h, nstep)
!#py      end if
!#py 
!#py      ! Accumulate 24-hour running mean of wind speed
!#py      do t = begt,endt
!#py         rbufslt(t) = this%windbot(t)
!#py      end do
!#py      if (use_fates) then
!#py         call update_accum_field  ('WIND24H', rbufslt, nstep)
!#py         call extract_accum_field ('WIND24H', this%wind24h, nstep)
!#py      end if
!#py 
!#py      deallocate(rbufslt)
!#py !#py     end subroutine update_acc_vars_top_as
  ...
  subroutine init_top_af(this, begt, endt)
    ...
    !#py call hist_addfld1d (fname='RAIN', units='mm/s',  &
         !#py avgflag='A', long_name='atmospheric rain', &
         !#py !#py ptr_topo=this%rain,t2g_scale_type='unity')

    this%snow(begt:endt) = spval
    !#py call hist_addfld1d (fname='SNOW', units='mm/s',  &
         !#py avgflag='A', long_name='atmospheric snow', &
         !#py !#py ptr_topo=this%snow,t2g_scale_type='unity')

    this%solar(begt:endt) = spval
    !#py call hist_addfld1d (fname='FSDS', units='W/m^2',  &
         !#py avgflag='A', long_name='atmospheric incident solar radiation', &
         !#py !#py ptr_topo=this%solar,t2g_scale_type='unity')

    this%lwrad(begt:endt) = spval
    !#py call hist_addfld1d (fname='FLDS', units='W/m^2',  &
         !#py avgflag='A', long_name='atmospheric longwave radiation', &
         !#py !#py ptr_topo=this%lwrad,t2g_scale_type='unity')
  ...
  subroutine init_acc_buffer_top_af (this, bounds)
    ...
    if (use_cn) then
       !#py call init_accum_field (name='PREC10D', units='MM H2O/S', &
          !#py desc='10-day running mean of total precipitation', accum_type='runmean', accum_period=-10, &
           !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)
       !#py call init_accum_field (name='PREC60D', units='MM H2O/S', &
          !#py desc='60-day running mean of total precipitation', accum_type='runmean', accum_period=-60, &
           !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)
    end if
    ! Accumulator flux variables used by FATES
    if (use_fates) then
        !#py call init_accum_field (name='PREC24H', units='MM H2O/S', &
             !#py desc='24hr running mean of total precipitation', accum_type='runmean', accum_period=-1, &
              !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)
    end if
    ! Accumulator variables for radiation fluxes
    !#py call init_accum_field (name='FSD24H', units='W/m2',                                             &
         !#py desc='24hr average of direct solar radiation',  accum_type='runmean', accum_period=-1,    &
         !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)

    !#py call init_accum_field (name='FSD240H', units='W/m2',                                            &
         !#py desc='240hr average of direct solar radiation',  accum_type='runmean', accum_period=-10,  &
         !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)

    !#py call init_accum_field (name='FSI24H', units='W/m2',                                             &
         !#py desc='24hr average of diffuse solar radiation',  accum_type='runmean', accum_period=-1,   &
         !#py !#py subgrid_type='topounit', numlev=1, init_value=0._r8)

    !#py call init_accum_field (name='FSI240H', units='W/m2',                                            &
  ...
  subroutine init_acc_vars_top_af(this, bounds)
    ...
    !#py call extract_accum_field ('FSD24H', rbufslt, nstep)
    this%fsd24h(begt:endt) = rbufslt(begt:endt)

    !#py call extract_accum_field ('FSD240H', rbufslt, nstep)
    this%fsd240h(begt:endt) = rbufslt(begt:endt)

    !#py call extract_accum_field ('FSI24H', rbufslt, nstep)
    this%fsi24h(begt:endt) = rbufslt(begt:endt)

    !#py call extract_accum_field ('FSI240H', rbufslt, nstep)
    this%fsi240h(begt:endt) = rbufslt(begt:endt)

    if (use_cn) then
        !#py call extract_accum_field ('PREC10D', rbufslt, nstep)
       this%prec10d(begt:endt) = rbufslt(begt:endt)

        !#py call extract_accum_field ('PREC60D', rbufslt, nstep)
       this%prec60d(begt:endt) = rbufslt(begt:endt)
    end if

    if (use_fates) then
        !#py call extract_accum_field ('PREC24H', rbufslt, nstep)
  ...
  !-----------------------------------------------------------------------
!#py   subroutine update_acc_vars_top_af (this, bounds)
!#py     !
!#py     ! USES
!#py     !#py use clm_time_manager, only : get_nstep
!#py     !#py use accumulMod      , only : update_accum_field, extract_accum_field
!#py     !
!#py     ! !ARGUMENTS:
!#py     class(topounit_atmospheric_flux)    :: this
!#py     type(bounds_type)      , intent(in) :: bounds
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py     integer :: g,t,c,p                   ! indices
...
!#py     endif
!#py 
!#py     ! Accumulate and extract forc_solad24 & forc_solad240
!#py     do t = begt,endt
!#py        rbufslt(t) = this%solad(t,1)
!#py     end do
!#py     call update_accum_field  ('FSD240H', rbufslt              , nstep)
!#py     call extract_accum_field ('FSD240H', this%fsd240h         , nstep)
!#py     call update_accum_field  ('FSD24H' , rbufslt              , nstep)
!#py     call extract_accum_field ('FSD24H' , this%fsd24h          , nstep)
!#py 
!#py     ! Accumulate and extract forc_solai24 & forc_solai240
!#py     do t = begt,endt
!#py        rbufslt(t) = this%solai(t,1)
!#py     end do
!#py     call update_accum_field  ('FSI24H' , rbufslt              , nstep)
!#py     call extract_accum_field ('FSI24H' , this%fsi24h          , nstep)
!#py     call update_accum_field  ('FSI240H', rbufslt              , nstep)
!#py     call extract_accum_field ('FSI240H', this%fsi240h         , nstep)
!#py 
!#py     ! Accumulate and extract total precip
!#py     do t = begt,endt
!#py        rbufslt(t) = this%rain(t) + this%snow(t)
!#py     end do
!#py     if (use_cn) then
!#py        ! Accumulate and extract PREC60D (accumulates total precipitation as 60-day running mean)
!#py        call update_accum_field  ('PREC60D', rbufslt, nstep)
!#py        call extract_accum_field ('PREC60D', this%prec60d, nstep)
!#py 
!#py        ! Accumulate and extract PREC10D (accumulates total precipitation as 10-day running mean)
!#py        call update_accum_field  ('PREC10D', rbufslt, nstep)
!#py        call extract_accum_field ('PREC10D', this%prec10d, nstep)
!#py     end if
!#py 
!#py     if (use_fates) then
!#py        call update_accum_field  ('PREC24H', rbufslt, nstep)
!#py        call extract_accum_field ('PREC24H', this%prec24h, nstep)
!#py     end if
!#py 
!#py     deallocate(rbufslt)
!#py 
!#py !#py   end subroutine update_acc_vars_top_af
  ...
  !-----------------------------------------------------------------------
!#py   subroutine restart_top_es(this, bounds, ncid, flag)
!#py     class(topounit_energy_state) :: this
!#py     type(bounds_type), intent(in) :: bounds
...
!#py 
!#py     call restartvar(ncid=ncid, flag=flag, varname='TS_TOPO', xtype=ncd_double, &
!#py         dim1name='topounit', long_name='surface radiative temperature', &
!#py         units='K', interpinic_flag='copy', readvar=readvar, data=this%t_rad)
!#py 
!#py !#py !#py   end subroutine restart_top_es
