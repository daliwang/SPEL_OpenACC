...
contains
  ...
  !-----------------------------------------------------------------------
!#py   subroutine C14_init_BombSpike()
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! read netcdf file containing a timeseries of atmospheric delta C14 values; save in module-level array
!#py     !
!#py     ! !USES:
!#py     use ncdio_pio
...
!#py     integer :: dimid,varid                ! input netCDF id's
!#py     integer :: ntim                       ! number of input data time samples
!#py     integer :: t
!#py     !-----------------------------------------------------------------------
!#py 
!#py     if ( use_c14_bombspike ) then
!#py 
!#py        if ( masterproc ) then
!#py           write(iulog, *) 'C14_init_BombSpike: preparing to open file:'
!#py           write(iulog, *) trim(locfn)
!#py        endif
!#py 
!#py        call getfil(atm_c14_filename, locfn, 0)
!#py 
!#py        call ncd_pio_openfile (ncid, trim(locfn), 0)
!#py 
!#py        call ncd_inqdlen(ncid,dimid,ntim,'time')
!#py 
!#py        !! allocate arrays based on size of netcdf timeseries
!#py        allocate(atm_c14file_time(ntim))
!#py        allocate(atm_delta_c14(ntim))
!#py 
!#py        call ncd_io(ncid=ncid, varname='time', flag='read', data=atm_c14file_time)
!#py 
!#py        call ncd_io(ncid=ncid, varname='atm_delta_c14', flag='read', data=atm_delta_c14)
!#py 
!#py        call ncd_pio_closefile(ncid)
!#py 
!#py        ! check to make sure that time dimension is well behaved
!#py        do t = 2, ntim
!#py           if ( atm_c14file_time(t) - atm_c14file_time(t-1) <= 0._r8 ) then
!#py              write(iulog, *) 'C14_init_BombSpike: error.  time axis must be monotonically increasing'
!#py              call endrun(msg=errMsg(__FILE__, __LINE__))
!#py           endif
!#py        end do
!#py     endif
!#py 
!#py   end subroutine C14_init_BombSpike
