...
contains
  ...
  subroutine InitHistory(this, bounds)
    ...
    !#py call hist_addfld1d (fname='BTRAN', units='1',  &
         !#py avgflag='A', long_name='transpiration beta factor', &
         !#py !#py ptr_patch=this%btran_patch, set_lake=spval, set_urb=spval)

    if (use_cn) then
       this%rresis_patch(begp:endp,:) = spval
       !#py call hist_addfld2d (fname='RRESIS', units='proportion', type2d='levgrnd', &
            !#py avgflag='A', long_name='root resistance in each soil layer', &
            !#py !#py ptr_patch=this%rresis_patch, default='inactive')
    end if
...
!#py     !-----------------------------------------------------------------------
!#py 
!#py     call restartvar(ncid=ncid, flag=flag, varname='btran2', xtype=ncd_double,  &
!#py          dim1name='pft', &
!#py          long_name='', units='', &
!#py          interpinic_flag='interp', readvar=readvar, data=this%btran2_patch) 
!#py 
!#py     call this%eflx_dynbal_dribbler%Restart(bounds, ncid, flag)
!#py 
