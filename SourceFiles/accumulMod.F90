module accumulMod
  ...
  !#py public :: accumulRest          ! Write/read restart of accumulation fields
  public :: init_accum_field     ! Initialize an accumulator field
  !#py public :: print_accum_fields   ! Print info about accumulator fields
...
contains
  ...
  subroutine init_accum_field (name, units, desc, &
    ...
    if (naccflds > max_accum) then
       !#py write(iulog,*) 'ACCUMULINIT error: user-defined accumulation fields ', &
            !#py 'equal to ',naccflds,' exceeds max_accum'
       !#py call shr_sys_abort()
    end if
    nf = naccflds
    ...
    if (accum(nf)%period < 0) then
       accum(nf)%period = -accum(nf)%period * nint(SHR_CONST_CDAY) / dtime_mod
    end if
    ...
    case ('pft')
       ...
       num1d = nump
    case default
       !#py write(iulog,*)'ACCUMULINIT: unknown subgrid type ',subgrid_type
       !#py call shr_sys_abort ()
    end select
 ...
 subroutine print_accum_fields()
   ...
   if (1) then
      !#py write(iulog,*)
      !#py write(iulog,*) 'Initializing variables for time accumulation .....'
      !#py write(iulog,'(72a1)') ("-",i=1,60)
      !#py write(iulog,*) 'Accumulated fields'
      !#py write(iulog,1002)
      !#py write(iulog,'(72a1)') ("_",i=1,71)
      do nf = 1, naccflds
         if (accum(nf)%period /= huge(1)) then
            !#py write(iulog,1003) nf,accum(nf)%name,accum(nf)%units,&
                 !#py accum(nf)%acctype, accum(nf)%period, accum(nf)%initval, &
                 !#py accum(nf)%desc
         else
            !#py write(iulog,1004) nf,accum(nf)%name,accum(nf)%units,&
                 !#py accum(nf)%acctype, accum(nf)%initval, accum(nf)%desc
         endif
      end do
      !#py write(iulog,'(72a1)') ("_",i=1,71)
      !#py write(iulog,*)
      !#py write(iulog,'(72a1)') ("-",i=1,60)
      !#py write(iulog,*) 'Successfully initialized variables for accumulation'
      !#py write(iulog,*)
   endif
  ...
  subroutine extract_accum_field_sl (name, field, nstep)
    ...
    if (nf == 0) then
       !#py write(iulog,*) 'EXTRACT_ACCUM_FIELD_SL error: field name ',name,' not found'
       !#py call shr_sys_abort
    endif
    ...
    if (size(field,dim=1) /= end-beg+1) then
       !#py write(iulog,*)'ERROR in extract_accum_field for field ',accum(nf)%name
       !#py write(iulog,*)'size of first dimension of field is ',&
            !#py size(field,dim=1),' and should be ',end-beg+1
       !#py call shr_sys_abort
    endif
  ...
  subroutine extract_accum_field_ml (name, field, nstep)
    ...
    if (nf == 0) then
       !#py write(iulog,*) 'EXTRACT_ACCUM_FIELD_ML error: field name ',name,' not found'
       !#py call shr_sys_abort
    endif
    ...
    if (size(field,dim=1) /= end-beg+1) then
       ...
       !#py write(iulog,*)'ERROR in extract_accum_field for field ',accum(nf)%name
       !#py write(iulog,*)'size of second dimension of field iis ',&
            !#py size(field,dim=2),' and should be ',numlev
       !#py call shr_sys_abort
    endif
  ...
  subroutine update_accum_field_sl (name, field, nstep)
    ...
    if (nf == 0) then
       !#py write(iulog,*) 'UPDATE_ACCUM_FIELD_SL error: field name ',name,' not found'
       !#py call shr_sys_abort
    endif
    ...
    if (size(field,dim=1) /= end-beg+1) then
       !#py write(iulog,*)'ERROR in UPDATE_ACCUM_FIELD_SL for field ',accum(nf)%name
       !#py write(iulog,*)'size of first dimension of field is ',size(field,dim=1),&
            !#py ' and should be ',end-beg+1
       !#py call shr_sys_abort
    endif
    ...
    if (accum(nf)%acctype /= 'timeavg' .AND. &
       ...
       !#py write(iulog,*) 'UPDATE_ACCUM_FIELD_SL error: incorrect accumulation type'
       !#py write(iulog,*) ' was specified for field ',name
       !#py write(iulog,*)' accumulation type specified is ',accum(nf)%acctype
       !#py write(iulog,*)' only [timeavg, runmean, runaccum] are currently acceptable'
       !#py call shr_sys_abort()
    end if
  ...
  subroutine update_accum_field_ml (name, field, nstep)
    ...
    if (nf == 0) then
       !#py write(iulog,*) 'UPDATE_ACCUM_FIELD_ML error: field name ',name,' not found'
       !#py call shr_sys_abort
    endif
    ...
    if (size(field,dim=1) /= end-beg+1) then
       !#py write(iulog,*)'ERROR in UPDATE_ACCUM_FIELD_ML for field ',accum(nf)%name
       !#py write(iulog,*)'size of first dimension of field is ',size(field,dim=1),&
            !#py ' and should be ',end-beg+1
       !#py call shr_sys_abort
    else if (size(field,dim=2) /= numlev) then
       !#py write(iulog,*)'ERROR in UPDATE_ACCUM_FIELD_ML for field ',accum(nf)%name
       !#py write(iulog,*)'size of second dimension of field is ',size(field,dim=2),&
            !#py ' and should be ',numlev
       !#py call shr_sys_abort
    endif
    ...
    if (accum(nf)%acctype /= 'timeavg' .AND. &
        accum(nf)%acctype /= 'runmean' .AND. &
        accum(nf)%acctype /= 'runaccum') then
       !#py write(iulog,*) 'UPDATE_ACCUM_FIELD_ML error: incorrect accumulation type'
       !#py write(iulog,*) ' was specified for field ',name
       !#py write(iulog,*)' accumulation type specified is ',accum(nf)%acctype
       !#py write(iulog,*)' only [timeavg, runmean, runaccum] are currently acceptable'
       !#py call shr_sys_abort()
  ...
  !------------------------------------------------------------------------
!#py   subroutine accumulRest( ncid, flag )
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Read/write accumulation restart data
!#py     !
!#py     ! !USES:
!#py     use clm_varcon      , only : ispval
!#py     use restUtilMod     , only : restartvar
!#py     use ncdio_pio       , only : file_desc_t, ncd_double, ncd_int
!#py     !
!#py     ! !ARGUMENTS:
!#py     implicit none
!#py     type(file_desc_t), intent(inout) :: ncid   !netcdf unit
!#py     character(len=*) , intent(in) :: flag   !'define','read', or 'write'
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py     integer :: nf                             ! indices
!#py     logical :: readvar                        ! determine if variable is on initial file
!#py     character(len=128) :: varname             ! temporary
!#py     character(len= 32) :: subname='AccumRest' ! subroutine name
!#py     !------------------------------------------------------------------------
!#py
!#py     do nf = 1,naccflds
!#py
!#py        ! Note = below need to allocate rbuf for single level variables, since
!#py        ! accum(nf)%val is always 2d
!#py
!#py        varname = trim(accum(nf)%name) // '_VALUE'
!#py        if (accum(nf)%numlev == 1) then
!#py           call restartvar(ncid=ncid, flag=flag, varname=varname, xtype=ncd_double, &
!#py                dim1name=accum(nf)%type1d, &
!#py                long_name=accum(nf)%desc, units=accum(nf)%units, &
!#py                interpinic_flag='interp', &
!#py                data=accum(nf)%val, readvar=readvar)
!#py        else
!#py           call restartvar(ncid=ncid, flag=flag, varname=varname, xtype=ncd_double, &
!#py                dim1name=accum(nf)%type1d, dim2name=accum(nf)%type2d, &
!#py                long_name=accum(nf)%desc, units=accum(nf)%units, &
!#py                interpinic_flag='interp', &
!#py                data=accum(nf)%val, readvar=readvar)
!#py        end if
!#py
!#py        varname = trim(accum(nf)%name) // '_PERIOD'
!#py        call restartvar(ncid=ncid, flag=flag, varname=varname, xtype=ncd_int, &
!#py             long_name='', units='time steps', &
!#py             imissing_value=ispval, ifill_value=huge(1), &
!#py             interpinic_flag='copy', &
!#py             data=accum(nf)%period, readvar=readvar)
!#py
!#py     end do
!#py
!#py   end subroutine accumulRest
