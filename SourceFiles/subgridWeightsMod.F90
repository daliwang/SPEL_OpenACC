...
!#py #include "shr_assert.h"
  ...
  !#py public :: init_subgrid_weights_mod      ! initialize stuff in this module
...
contains
  ...
  !-----------------------------------------------------------------------
!#py   subroutine init_subgrid_weights_mod(bounds)
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Initialize stuff in this module
!#py     !
!#py     ! !USES:
!#py     use landunit_varcon, only : max_lunit
!#py     use elm_varpar     , only : maxpatch_glcmec, natpft_size, cft_size
!#py     use shr_infnan_mod , only : nan => shr_infnan_nan, assignment(=)
!#py     use decompMod      , only : BOUNDS_LEVEL_PROC
!#py     use histFileMod    , only : hist_addfld2d
!#py     !
!#py     ! !ARGUMENTS:
!#py     type(bounds_type), intent(in) :: bounds  ! proc bounds
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py 
!#py     character(len=*), parameter :: subname = 'init_subgrid_weights_mod'
!#py     !-----------------------------------------------------------------------
!#py 
!#py     SHR_ASSERT(bounds%level == BOUNDS_LEVEL_PROC, errMsg(__FILE__, __LINE__))
!#py 
!#py     ! ------------------------------------------------------------------------
!#py     ! Allocate variables in subgrid_weights_diagnostics
!#py     ! ------------------------------------------------------------------------
!#py 
!#py     ! Note that, because these variables are output to the history file, it appears that
!#py     ! their lower bounds need to start at 1 (e.g., 1:natpft_size rather than
!#py     ! natpft_lb:natpft_ub)
!#py     allocate(subgrid_weights_diagnostics%pct_landunit(bounds%begg:bounds%endg, 1:max_lunit))
!#py     subgrid_weights_diagnostics%pct_landunit(:,:) = nan
!#py     allocate(subgrid_weights_diagnostics%pct_nat_pft(bounds%begg:bounds%endg, 1:natpft_size))
!#py     subgrid_weights_diagnostics%pct_nat_pft(:,:) = nan
!#py     allocate(subgrid_weights_diagnostics%pct_cft(bounds%begg:bounds%endg, 1:cft_size))
!#py     subgrid_weights_diagnostics%pct_cft(:,:) = nan
!#py     allocate(subgrid_weights_diagnostics%pct_glc_mec(bounds%begg:bounds%endg, 1:maxpatch_glcmec))
!#py     subgrid_weights_diagnostics%pct_glc_mec(:,:) = nan
!#py 
!#py     ! ------------------------------------------------------------------------
!#py     ! Add history fields
!#py     ! ------------------------------------------------------------------------
!#py 
!#py     call hist_addfld2d (fname='PCT_LANDUNIT', units='%', type2d='ltype', &
!#py          avgflag='A', long_name='% of each landunit on grid cell', &
!#py          ptr_lnd=subgrid_weights_diagnostics%pct_landunit)
!#py 
!#py     call hist_addfld2d (fname='PCT_NAT_PFT', units='%', type2d='natpft', &
!#py          avgflag='A', long_name='% of each PFT on the natural vegetation (i.e., soil) landunit', &
!#py          ptr_lnd=subgrid_weights_diagnostics%pct_nat_pft)
!#py 
!#py     if (cft_size > 0) then
!#py        call hist_addfld2d (fname='PCT_CFT', units='%', type2d='cft', &
!#py             avgflag='A', long_name='% of each crop on the crop landunit', &
!#py             ptr_lnd=subgrid_weights_diagnostics%pct_cft)
!#py     end if
!#py 
!#py     if (maxpatch_glcmec > 0) then
!#py        call hist_addfld2d (fname='PCT_GLC_MEC', units='%', type2d='glc_nec', &
!#py             avgflag='A', long_name='% of each GLC elevation class on the glc_mec landunit', &
!#py             ptr_lnd=subgrid_weights_diagnostics%pct_glc_mec)
!#py     end if
!#py 
!#py 
!#py   end subroutine init_subgrid_weights_mod
...
#ifndef _OPENACC
       !#py write(iulog,*) subname//' ERROR: Attempt to assign non-zero weight to a non-existent landunit'
       !#py write(iulog,*) 'g, t, l, ltype, weight = ', top_pp%gridcell(t), t, l, ltype, weight
  ...
  subroutine check_weights (bounds, active_only)
    ...
    do c = bounds%begc,bounds%endc
       if (.not. weights_okay(sumwtcol(c), active_only, col_pp%active(c))) then
          !#py write(iulog,*) trim(subname),' ERROR: at c = ',c,'total PFT weight is ',sumwtcol(c), &
                         !#py 'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do l = bounds%begl,bounds%endl
       if (.not. weights_okay(sumwtlunit(l), active_only, lun_pp%active(l))) then
          !#py write(iulog,*) trim(subname),' ERROR: at l = ',l,'total PFT weight is ',sumwtlunit(l), &
                         !#py 'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do g = bounds%begg,bounds%endg
       if (.not. weights_okay(sumwtgcell(g), active_only, i_am_active=.true.)) then
          !#py write(iulog,*) trim(subname),' ERROR: at g = ',g,'total PFT weight is ',sumwtgcell(g), &
                         !#py 'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do l = bounds%begl,bounds%endl
       if (.not. weights_okay(sumwtlunit(l), active_only, lun_pp%active(l))) then
          !#py write(iulog,*) trim(subname),' ERROR: at l = ',l,'total col weight is ',sumwtlunit(l), &
                         !#py 'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do g = bounds%begg,bounds%endg
       if (.not. weights_okay(sumwtgcell(g), active_only, i_am_active=.true.)) then
          !#py write(iulog,*) trim(subname),' ERROR: at g = ',g,'total col weight is ',sumwtgcell(g), &
                         !#py 'active_only = ', active_only
          error_found = .true.
       end if
    end do
    ...
    do g = bounds%begg,bounds%endg
       if (.not. weights_okay(sumwtgcell(g), active_only, i_am_active=.true.)) then
          !#py write(iulog,*) trim(subname),' ERROR: at g = ',g,'total lunit weight is ',sumwtgcell(g), &
                         !#py 'active_only = ', active_only
          error_found = .true.
       end if
    end do
