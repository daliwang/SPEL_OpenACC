...
CONTAINS
  ...
  SUBROUTINE shr_orb_params( iyear_AD , eccen  , obliq , mvelp     ,     &
    ...
    !#py if ( log_print .and. s_loglev > 0 ) then
    !#py    !#py write(s_logunit,F00) 'Calculate characteristics of the orbit:'
    !#py end if
    ...
    IF ( iyear_AD == SHR_ORB_UNDEF_INT ) THEN
       ...
       if( obliq == SHR_ORB_UNDEF_REAL )then
          !#py write(s_logunit,F00) trim(subname)//' Have to specify orbital parameters:'
          !#py write(s_logunit,F00) 'Either set: iyear_AD, OR [obliq, eccen, and mvelp]:'
          !#py write(s_logunit,F00) 'iyear_AD is the year to simulate orbit for (ie. 1950): '
          !#py write(s_logunit,F00) 'obliq, eccen, mvelp specify the orbit directly:'
          !#py write(s_logunit,F00) 'The AMIP II settings (for a 1995 orbit) are: '
          !#py !#py write(s_logunit,F00) ' obliq =  23.4441'
          !#py !#py write(s_logunit,F00) ' eccen =   0.016715'
          !#py !#py write(s_logunit,F00) ' mvelp = 102.7'
          !#py call shr_sys_abort(subname//' ERROR: unreasonable obliq')
       else if ( log_print ) then
          !#py write(s_logunit,F00) 'Use input orbital parameters: '
       end if
       if( (obliq < SHR_ORB_OBLIQ_MIN).or.(obliq > SHR_ORB_OBLIQ_MAX) ) then
          !#py write(s_logunit,F03) 'Input obliquity unreasonable: ', obliq
          !#pycall shr_sys_abort(subname//' ERROR: unreasonable obliq')
       end if
       if( (eccen < SHR_ORB_ECCEN_MIN).or.(eccen > SHR_ORB_ECCEN_MAX) ) then
          !#py write(s_logunit,F03) 'Input eccentricity unreasonable: ', eccen
          !#pycall shr_sys_abort(subname//' ERROR: unreasonable eccen')
       end if
       if( (mvelp < SHR_ORB_MVELP_MIN).or.(mvelp > SHR_ORB_MVELP_MAX) ) then
          !#py write(s_logunit,F03) 'Input mvelp unreasonable: ' , mvelp
          !#pycall shr_sys_abort(subname//' ERROR: unreasonable mvelp')
       end if
       eccen2 = eccen*eccen
       eccen3 = eccen2*eccen
    ...
    ELSE  ! Otherwise calculate based on years before present
       ...
       !#py if ( log_print .and. s_loglev > 0) then
      !#py     !#py write(s_logunit,F01) 'Calculate orbit for year: ' , iyear_AD
       !#py end if
       yb4_1950AD = 1950.0_SHR_KIND_R8 - real(iyear_AD,SHR_KIND_R8)
       if ( abs(yb4_1950AD) .gt. 1000000.0_SHR_KIND_R8 )then
          !#py write(s_logunit,F00) 'orbit only valid for years+-1000000'
          !#py write(s_logunit,F00) 'Relative to 1950 AD'
          !#py write(s_logunit,F03) '# of years before 1950: ',yb4_1950AD
          !#py write(s_logunit,F01) 'Year to simulate was  : ',iyear_AD
          !#pycall shr_sys_abort(subname//' ERROR: unreasonable year')
       end if
    ...
    if ( log_print ) then
       !#py write(s_logunit,F03) '------ Computed Orbital Parameters ------'
       !#py !#py write(s_logunit,F03) 'Eccentricity      = ',eccen
       !#py !#py write(s_logunit,F03) 'Obliquity (deg)   = ',obliq
       !#py !#py write(s_logunit,F03) 'Obliquity (rad)   = ',obliqr
       !#py !#py write(s_logunit,F03) 'Long of perh(deg) = ',mvelp
       !#py !#py write(s_logunit,F03) 'Long of perh(rad) = ',mvelpp
       !#py !#py write(s_logunit,F03) 'Long at v.e.(rad) = ',lambm0
       !#py write(s_logunit,F03) '-----------------------------------------'
    end if
  ...
  SUBROUTINE shr_orb_print( iyear_AD, eccen, obliq, mvelp )
    ...
    !#py if (s_loglev > 0) then
    !#py    if ( iyear_AD .ne. SHR_ORB_UNDEF_INT ) then
    !#py       if ( iyear_AD > 0 ) then
    !#py          !#py write(s_logunit,F01) 'Orbital parameters calculated for year: AD ',iyear_AD
    !#py       else
    !#py          !#py write(s_logunit,F01) 'Orbital parameters calculated for year: BC ',iyear_AD
    !#py       end if
    !#py    else if ( obliq /= SHR_ORB_UNDEF_REAL ) then
    !#py       !#py write(s_logunit,F03) 'Orbital parameters: '
    !#py       !#py write(s_logunit,F03) 'Obliquity (degree):              ', obliq
    !#py       !#py write(s_logunit,F03) 'Eccentricity (unitless):         ', eccen
    !#py       !#py write(s_logunit,F03) 'Long. of moving Perhelion (deg): ', mvelp
    !#py    else
    !#py       !#py write(s_logunit,F03) 'Orbit parameters not set!'
    !#py    end if
    !#py endif
