module LandunitDataType
  ...
  contains
    procedure, public :: Init    => lun_es_init
     !#py procedure, public :: Restart => lun_es_restart
  ...
  contains
    procedure, public :: Init    => lun_ws_init
     !#py procedure, public :: Restart => lun_ws_restart
...
contains
  ...
  subroutine lun_es_init(this, begl, endl)
    ...
    this%t_building(begl:endl) = spval
     !#py call hist_addfld1d(fname='TBUILD', units='K',  &
          !#py avgflag='A', long_name='internal urban building temperature', &
           !#py !#py ptr_lunit=this%t_building, set_nourb=spval, l2g_scale_type='unity')
  ...
  !------------------------------------------------------------------------
!#py    subroutine lun_es_restart(this, bounds, ncid, flag)
!#py      !
!#py      ! !DESCRIPTION:
!#py      ! Read/Write landunit energy state information to/from restart file.
!#py      !
!#py      ! !USES:
!#py      !
!#py      ! !ARGUMENTS:
!#py      class(landunit_energy_state) :: this
...
!#py      !-----------------------------------------------------------------------
!#py 
!#py      call restartvar(ncid=ncid, flag=flag, varname='taf', xtype=ncd_double, dim1name='landunit',                       &
!#py           long_name='urban canopy air temperature', units='K',                                                         &
!#py           interpinic_flag='interp', readvar=readvar, data=this%taf)
!#py !#py !#py      end subroutine lun_es_restart
  ...
  !------------------------------------------------------------------------
!#py    subroutine lun_ws_restart(this, bounds, ncid, flag)
!#py      !
!#py      ! !DESCRIPTION:
!#py      ! Read/Write landunit water state information to/from restart file.
!#py      !
!#py      ! !USES:
!#py      !
!#py      ! !ARGUMENTS:
!#py      class(landunit_water_state)      :: this
!#py      type(bounds_type), intent(in)    :: bounds
!#py      type(file_desc_t), intent(inout) :: ncid
!#py      character(len=*) , intent(in)    :: flag
!#py      !
...
!#py      !-----------------------------------------------------------------------
!#py 
!#py      call restartvar(ncid=ncid, flag=flag, varname='qaf', xtype=ncd_double, dim1name='landunit',                       &
!#py           long_name='urban canopy specific humidity', units='kg/kg',                                                   &
!#py           interpinic_flag='interp', readvar=readvar, data=this%qaf)
!#py 
!#py !#py !#py      end subroutine lun_ws_restart
