...
!#py     !
!#py     ! !ARGUMENTS:
!#py     class(glc2lnd_type) :: this
!#py     type(bounds_type), intent(in) :: bounds
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py     integer  :: begg, endg
!#py 
!#py     character(len=*), parameter :: subname = 'InitHistory'
!#py     !-----------------------------------------------------------------------
!#py 
!#py     begg = bounds%begg
!#py     endg = bounds%endg
...
!#py        this%icemask_grc(begg:endg) = spval
!#py        call hist_addfld1d (fname='ICE_MASK',  units='1',  &
!#py             avgflag='I', long_name='Ice sheet mask coverage', &
!#py             ptr_gcell=this%icemask_grc)
!#py     end if
!#py 
...
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Read/Write glc2lnd information to/from restart file.
!#py     !
!#py     ! !USES:
!#py     use ncdio_pio , only : ncd_double, file_desc_t
!#py     use decompMod , only : bounds_type
!#py     use restUtilMod
!#py     !
!#py     ! !ARGUMENTS:
!#py     class(glc2lnd_type) , intent(inout) :: this
!#py     type(bounds_type)   , intent(in)    :: bounds
...
!#py     !-----------------------------------------------------------------------
!#py 
!#py     call restartvar(ncid=ncid, flag=flag, varname='icemask', xtype=ncd_double, &
!#py          dim1name='gridcell', &
!#py          long_name='total ice-sheet grid coverage mask', units='fraction', &
!#py          interpinic_flag='skip', readvar=readvar, data=this%icemask_grc)
!#py 
            ...
            do t = grc_pp%topi(g), grc_pp%topf(g)
               ...
               if (area_ice_mec > 0) then
                  ...
                  if (l_ice_mec == ispval) then
                     !#py write(iulog,*) ' ERROR: no ice_mec landunit found within the icemask, for g = ', g
                     !#py call endrun()
                  end if

                  frac_assigned(1:maxpatch_glcmec) = .false.
                  do c = lun_pp%coli(l_ice_mec), lun_pp%colf(l_ice_mec)
                     icemec_class = col_pp%itype(c) - istice_mec*100 !col_itype_to_icemec_class(col_pp%itype(c))
  ...
  subroutine check_glc2lnd_icemask(this, bounds)
    ...
    do g = bounds%begg, bounds%endg
       ...
       if (this%icemask_grc(g) > 0._r8 .and. ldomain%glcmask(g) == 0._r8) then
          !#py write(iulog,*) subname//' ERROR: icemask must be a subset of glcmask.'
          !#py write(iulog,*) 'You can fix this problem by adding more grid cells'
          !#py write(iulog,*) 'to the mask defined by the fglcmask file.'
          !#py write(iulog,*) '(Change grid cells to 1 everywhere that CISM can operate.)'
          !#py !#py call endrun(decomp_index=g, elmlevel=nameg, msg=errMsg(__FILE__, __LINE__))
       end if
    end do
  ...
  subroutine check_glc2lnd_icemask_coupled_fluxes(this, bounds)
    ...
    do g = bounds%begg, bounds%endg
       ...
       if (this%icemask_coupled_fluxes_grc(g) > 0._r8 .and. this%icemask_grc(g) == 0._r8) then
          !#py write(iulog,*) subname//' ERROR: icemask_coupled_fluxes must be a subset of icemask.'
          !#py !#py call endrun(decomp_index=g, elmlevel=nameg, msg=errMsg(__FILE__, __LINE__))
       end if
    end do
       ...
       if (this%icemask_grc(g) > 0._r8) then
          do t = grc_pp%topi(g), grc_pp%topf(g)
             ...
             if (area_ice_mec > 0) then
                ...
                if (error) then
                   !#py write(iulog,*) subname//' ERROR: at least one glc_mec column has non-zero area from the coupler,'
                   !#py write(iulog,*) 'but there was no slot in memory for this column; g = ', g
                   !#py write(iulog,*) 'this%frac_grc(g, 1:maxpatch_glcmec) = ', &
                        !#py this%frac_grc(g, 1:maxpatch_glcmec)
                   !#py write(iulog,*) 'frac_assigned(1:maxpatch_glcmec) = ', &
                        !#py frac_assigned(1:maxpatch_glcmec)
                   !#py call endrun()
                end if  ! error
             end if  ! area_ice_mec > 0
          end do ! loop over topounits on this gridcell
